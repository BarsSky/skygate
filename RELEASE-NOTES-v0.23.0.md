...
3. waits ~15s
4. sees a green "Provisioned: container headscale-admin is
   listening on http://headscale-admin:50451..." flash
...
- **their own sqlite DB** (bind-mounted to
  `/home/admin/headscale/users/<username>/data/`)
- **their own config** (bind-mounted to
  `/home/admin/headscale/users/<username>/config/`)
...
4. generates a `config.yaml` with the per-user base_domain
   (`<username>.tsnet.example.com`) and MagicDNS enabled
...
3. MOVES the per-user directory to
   `/home/admin/headscale/.decommissioned-<username>-<ts>/`
...
Phase 1 is **infrastructure only**. No data migration. admin
still uses the global headscale for everything; the per-user
headscale exists, has admin as a user, has the API key
...
admin calls `HSForUser(1).ListAllNodes()`, it'll see an
...
- **v0.23.1 (Phase 2)** — migrate all 4 prod users to per-plane
  (user1, guest, user2 in addition to admin). Two
...
7. DB: portal_users.headscale_url = 'http://headscale-admin:50451'
...
10. per-user headscale has user 'admin' in its DB
