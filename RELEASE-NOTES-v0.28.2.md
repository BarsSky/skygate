...
Released 2026-07-25. Live on the operator's VM (192.168.13.69).
...
3. The re-apply added `tag:exit-emilia` /
   `tag:exit-sharlotta` / `tag:exit-karolina` to
...
4. `headscale nodes tag -i <id> -t
   ...
   on emilia / sharlotta / karolina — now accepted
...
- `headscale policy get`:
  ...
  - `grants with via: 3`
    - `skyadmin@...` → `via: ["tag:exit-emilia"]`
    - `michail@...` → `via: ["tag:exit-sharlotta"]`
    - `daniil@...` → `via: ["tag:exit-karolina"]`
  - `tagOwners`:
    - `tag:exit-emilia: ['skyadmin@...']`
    - `tag:exit-sharlotta: ['skyadmin@...']`
    - `tag:exit-karolina: ['skyadmin@...']`
  - `hosts: 212` (4 per-user + 208 per-device rule targets)
- `headscale nodes list -o json`:
  - `emilia (id=3): tags=['tag:exit-node', 'tag:exit-emilia', 'tag:public', 'tag:dev-skyadmin-emilia']`
  - `sharlotta (id=4): tags=['tag:exit-node', 'tag:exit-sharlotta', 'tag:public', 'tag:dev-skyadmin-sharlotta']`
  - `karolina (id=11): tags=['tag:exit-node', 'tag:exit-karolina', 'tag:public', 'tag:dev-skyadmin-karolina']`
...
# expect: ['tag:exit-node', 'tag:exit-emilia', 'tag:exit-karolina', 'tag:exit-sharlotta']

# 4. emilia / sharlotta / karolina have per-exit-node tags applied
docker exec headscale headscale nodes list -o json | \
  python3 -c 'import json,sys; data=json.load(sys.stdin); \
  [print(n["givenName"], n["tags"]) for n in data if n.get("givenName") in ("emilia","sharlotta","karolina")]'
...
# expect: 3 rows (skyadmin/michail/daniil)
```
