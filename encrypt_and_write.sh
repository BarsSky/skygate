...
API_KEY=$(docker exec headscale-admin /ko-app/headscale apikeys list 2>/dev/null \
...
func main() {
	...
	defer db.Close()
	_, err = db.Exec(`UPDATE portal_users SET headscale_url = ?, headscale_api_key_enc = ? WHERE username = 'admin'`, url, enc)
	if err != nil { fmt.Fprintln(os.Stderr, "db write:", err); os.Exit(1) }
	fmt.Println("OK: wrote url + encrypted key to portal_users (admin)")
}
...
docker exec -e SKYGATE_SECRET_KEY="$SKYGATE_SECRET_KEY" \
    -w /app \
    skygate /usr/local/go/bin/go run ./cmd/_encrypt_tmp "$API_KEY" "http://headscale-admin:50451"
