...
# ------------------------------------------------------------------------------
# Contract G: v1.3.19.1 — <polygon-vm-hostname> is REMOVED entirely
# (<polygon-vm-hostname> / headscale node id=30 was the HA mirror, but the
...
if [ -n "${DSN:-}" ]; then
    ...
    cnt=$(echo "${out}" | tr -d '[:space:]')
    if [ "${cnt}" = "0" ]; then
        ok "live policy: 0 references to <polygon-vm-hostname> (v1.3.19.1 cleanup held)"
    else
    ...
    cnt=$(echo "${out}" | tr -d '[:space:]')
    if [ "${cnt}" = "0" ]; then
        ok "nom: 0 rows for tag:dev-infra-<polygon-vm-hostname> (v1.3.19.1 cleanup held)"
    else
    ...
    cnt=$(echo "${out}" | tr -d '[:space:]')
    if [ "${cnt}" = "0" ]; then
        ok "tagOwners: 0 entries for tag:dev-infra-<polygon-vm-hostname> (v1.3.19.1 cleanup held)"
    else
    ...
    if [ "${cnt}" = "4" ]; then
        ...
        bad "nom: ${cnt} tag:dev-infra-* rows (expected 4 after <polygon-vm-hostname> removal)"
    fi
fi
