...
# Pre-flight: remove any leftover smoke-test rules on device 3 (relay-1)
...
if [ -n "$ORPHAN_IDS" ]; then
  ...
  note "0. cleanup: removed $(echo $ORPHAN_IDS | wc -w) orphan smoke rules on device 3 (relay-1)"
fi
...
# Step 4: admin/* pages (admin is admin)
note "4. /admin/* pages"
...
if curl -s -b "$COOKIE" "$BASE/admin/users" | grep -q "admin"; then
...
# Use device 3 (relay-1) which has 0 manual rules so we don't hit the per-device 200 limit.
note "6. API: POST /my/exit-rules/api (add smoke-test rule on relay-1=3)"
RAND_VAL="198.51.100.$((RANDOM % 250 + 1))"
RESP=$(curl -s -b "$COOKIE" -X POST \
  -H "Content-Type: application/json" \
  -d "{\"rules\":[{\"device_id\":3,\"exit_node\":\"relay-3\",\"target_type\":\"subnet\",\"target_value\":\"$RAND_VAL/32\",\"action\":\"accept\"}]}" \
...
curl -s -c /tmp/smoke_new_ck -X POST -d "username=admin&password=${SMOKE_TEST_NEW_PASSWORD}" "$BASE/login" -o /dev/null
