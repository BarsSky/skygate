...
# (admin's tag:private device) from previous smoke runs. These were
...
# 3/4/11, which are exit-nodes relay-1/relay-2/relay-3) because the
...
for path in /admin/users /admin/devices /admin/audit /admin/acls \
            ...
            /admin/telegram /admin/headscale /admin/invites; do
  CODE=$(status "$BASE$path")
  [ "$CODE" = "200" ] && ok "$path 200" || bad "$path $CODE"
done
...
for path in /admin/users /admin/acls /admin/devices /admin/audit /admin/headscale /admin/invites; do
  HTML=$(html "$BASE$path")
...
# Content sanity: /admin/users must list admin
if html "$BASE/admin/users" | grep -q "admin"; then
  ok "/admin/users lists admin"
else
  bad "/admin/users: missing admin"
fi
...
# Use device 8 (admin's tag:private device) which has 0 manual rules
# so we don't hit the per-device 200 limit. NOT device 3 (relay-1),
# 4 (relay-2) or 11 (relay-3) — those are exit-nodes and the
...
RESP=$(curl -s "${ACCEPT_LANG_HDR[@]}" -b "$COOKIE" -X POST \
  ...
  -d "{\"rules\":[{\"device_id\":8,\"exit_node\":\"relay-3\",\"target_type\":\"subnet\",\"target_value\":\"$RAND_VAL/32\",\"action\":\"accept\"}]}" \
...
curl -s -c /tmp/smoke_new_ck "${ACCEPT_LANG_HDR[@]}" -X POST \
  --data-urlencode "username=admin" --data-urlencode "password=${SMOKE_TEST_NEW_PASSWORD}" \
