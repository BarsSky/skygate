...
#   SSH_HOST=skyadmin@1.2.3.4 bash scripts/rebuild_deploy.sh
#
...
$SSH 'sudo chown -R skyadmin:skyadmin /home/skyadmin/skygate/data/ts/ 2>/dev/null || true' || true
echo "[1/5] chown data/ts/ (silent if no permission denied)"

# 2. git pull
echo "[2/5] git pull --ff-only"
$SSH 'cd /home/skyadmin/skygate && git pull --ff-only' || {
