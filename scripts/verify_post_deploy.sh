...
#   R4  tailscaled running inside skygate-vm
#   R5  skygate-vm has tailnet IP 100.64.0.10
#   R6  skygate-vm does NOT use an exit-node (Docker bridge reachable)
#   R7  Docker bridge 172.18.0.0/16 reachable from skygate-vm
...
#   R17 emilia, sharlotta, karolina online in headscale
...
#   R22 https://skygate.skynas.ru/healthz → 200
...
run_vm_check "R4" "tailscaled running in skygate-vm" \
  "docker exec $SKYGATE_CONTAINER sh -c 'pgrep tailscaled | grep -q .'"
run_vm_check "R5" "skygate-vm tailnet IP = 100.64.0.10" \
  "docker exec $SKYGATE_CONTAINER tailscale --socket=/var/run/tailscale/tailscaled.sock status 2>&1 | grep -q '^100.64.0.10'"
# R6: skygate-vm must NOT use an exit-node. The Tailscale status line
...
EXITNODE_LINE=$(ssh_vm \
  "docker exec $SKYGATE_CONTAINER tailscale --socket=/var/run/tailscale/tailscaled.sock status 2>&1 | grep '^100.64.0.10'")
LAST_FIELD=$(echo "$EXITNODE_LINE" | awk '{print $NF}')
if [ "$LAST_FIELD" = "-" ]; then
  echo "  ${GRN}PASS${NC}  R6  skygate-vm has NO exit-node (last field = '-')"
  RESULTS_PASS=$((RESULTS_PASS+1))
else
...
n = sum(1 for g in p.get('grants',[])
        if any('@tsnet.skynas.ru' in s for s in g.get('src',[]))
...
for g in policy.get("grants", []):
    srcs = g.get("src", [])
    user_match = [s for s in srcs if "@tsnet.skynas.ru" in s]
    if not user_match:
        continue
    ...
    db_user = next((u for u in db.get("user_prefs", [])
                    if str(u.get("user_id")) in {"1":"skyadmin","6":"michail","9":"guest","10":"daniil"}.get(str(u.get("user_id")),"") or u.get("user_id") in (1,6,9,10) and u.get("tag") in (None,via) ), None)
    # Simpler: match by user_id→username map
    user_map = {"1":"skyadmin","6":"michail","9":"guest","10":"daniil"}
    db_user = None
...
if [ "$QUICK" = 0 ] && [ -n "$NODES_JSON" ]; then
  ...
  for host in emilia sharlotta karolina; do
...
if [ "$SKIP_NETWORK" = 0 ]; then
  echo
  echo "[R22-R24] HTTPS path (skygate.skynas.ru)"
  HTTPS_CODE=$(curl -sk -o /dev/null -w '%{http_code}' --max-time 10 https://skygate.skynas.ru/healthz 2>&1)
  if [ "$HTTPS_CODE" = "200" ]; then
    echo "  ${GRN}PASS${NC}  R22 https://skygate.skynas.ru/healthz → 200"
    RESULTS_PASS=$((RESULTS_PASS+1))
  else
    echo "  ${RED}FAIL${NC}  R22 https://skygate.skynas.ru/healthz → $HTTPS_CODE"
    RESULTS_FAIL=$((RESULTS_FAIL+1))
  fi

  # R23: TLS cert expiry (must be > 7 days from now)
  CERT_END=$(echo | openssl s_client -servername skygate.skynas.ru -connect skygate.skynas.ru:443 2>/dev/null | openssl x509 -noout -enddate 2>/dev/null | sed 's/^notAfter=//')
  if [ -n "$CERT_END" ]; then
...
if [ "$SKIP_NETWORK" = 0 ]; then
  echo
  echo "[R25] skygate-vm direct internet"
  # skygate-vm must reach 8.8.8.8 DIRECTLY (no exit-node). With
  ...
  if echo "$PING_LOSS" | grep -q '0%'; then
    echo "  ${GRN}PASS${NC}  R25 skygate-vm pings 8.8.8.8 (loss=$PING_LOSS)"
    RESULTS_PASS=$((RESULTS_PASS+1))
  else
