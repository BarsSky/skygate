...
if [ -f docs/oidc.md ]; then
    ok "docs/oidc.md exists"
else
    bad "docs/oidc.md MISSING (the runbook is the operator's only guide to wiring headscale)"
fi
...
for key in "SKYGATE_OIDC_ISSUER" "SKYGATE_OIDC_CLIENT_ID" "SKYGATE_OIDC_CLIENT_SECRET" "SKYGATE_OIDC_REDIRECT_URIS"; do
    if grep -qF "$key" docs/oidc.md; then
...
for step in "discovery" "jwks" "authorize"; do
    if grep -qiE "curl.*${step}|/\\.well-known/|/oidc/jwks\\.json|/oidc/authorize" docs/oidc.md; then
...
if grep -qE "Common e2e failures|common e2e failures" docs/oidc.md; then
...
if grep -qE "automatic_authorization: true" docs/oidc.md; then
