...
#
# Verifies the Phase 0 schema from docs/ha.md:
...
#   G) docs/ha.md exists with D1-D8 confirmed
...
if grep -qE "B195" "$PROJECT_DIR/AGENTS.md" 2>/dev/null; then
  ...
  no "AGENTS.md does not mention B195 (deferred to /docs/ha.md)"
fi
