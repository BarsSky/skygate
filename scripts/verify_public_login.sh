...
env PGPASSWORD=skygate_admin_pass psql -h 172.17.0.1 -p 5433 -U admin -d skygate_staging -tA -c "SELECT to_timestamp(created_at) AT TIME ZONE 'UTC' as ts, username, action, ip_address FROM audit_log WHERE action='login_ok' ORDER BY id DESC LIMIT 1"
