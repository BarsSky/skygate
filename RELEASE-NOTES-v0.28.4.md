...
v0.28.3 закрыл exit-node bypass, но как побочный эффект — msi (а
также другие устройства, наследующие `skyadmin`'s per-user via)
теперь pinned к **emilia**, а не к **karolina** (как раньше).
Оператор не мог это переопределить.

**v0.28.4** — per-device preferred exit-node. Конкретное устройство
(например msi) можно pinned к другому exit-node, нежели per-user
default. UI в `/my/devices` (self-service) и `/admin/devices` (admin
override).

- msi → `tag:exit-karolina` (specific override, перебивает skyadmin → emilia)
- skyworker (no override) → emilia (per-user default)
- karolina's 148 PrimaryRoutes по-прежнему недоступны msi (via
  constraint karolina≠emilia блокирует), но msi может **использовать
  karolina как exit-node** для autogroup:internet
...
```json
{ "src": ["tag:dev-skyadmin-msi"],
  "dst": ["autogroup:internet"],
  "ip":  ["*"],
  "via": ["tag:exit-karolina"] }
```

Порядок важен: msi's tag `tag:dev-skyadmin-msi` совпадает с per-device
grant ПЕРВЫМ (Tailscale first-match wins) → via=karolina. Без
per-device override — falls through к per-user grant (src=skyadmin@…
via=emilia).
...
=== per-device grants (autogroup:internet) ===
  src=tag:dev-skyadmin-msi  via=['tag:exit-karolina']  dst=['autogroup:internet']

=== per-user grants (with @tsnet) ===
  src=skyadmin@tsnet.skynas.ru  via=['tag:exit-emilia']     dst=[..., 'autogroup:internet']
  src=michail@tsnet.skynas.ru   via=['tag:exit-sharlotta'] dst=[..., 'autogroup:internet']
  src=daniil@tsnet.skynas.ru    via=['tag:exit-karolina']  dst=[..., 'autogroup:internet']
...
- **Unpinned state**: dropdown со списком доступных exit-nodes
  (emilia/sharlotta/karolina) + кнопка pin (◎)
...
3. Template uses `SkygateUserByHost[hostname]` → form `user_id=1` для msi.

Hotfix: первая попытка deploy'а показала "—" для msi (потому что
...
<form method="post" action="/my/devices/preferred-exit">
  ...
  <input type="hidden" name="tag" value="tag:exit-karolina">
  <button>Pin</button>
</form>
```
...
1. TestGenerateACLWithVia_PerDeviceGrantEmittedBeforePerUser
   - per-device grant (via=karolina) appears in grants[] BEFORE
     the per-user grant (via=emilia). Tailscale first-match ordering.
...
- 0 acls, **256 grants** (was 255 in v0.28.3; +1 for msi's per-device grant)
- 1 per-device grant: `tag:dev-skyadmin-msi → autogroup:internet via=[tag:exit-karolina]`
- 5 per-user grants: 3 with via (skyadmin→emilia, michail→sharlotta, daniil→karolina), 2 without
...
### /admin/devices (msi row)

```
id=15  msi  tagged-devices  100.64.0.11  tag:private  tag:dev-skyadmin-msi
      [PIN: tag:exit-karolina] [× clear]  online
```
...
4. Select `karolina` and click ◎ (pin)
5. Page reloads with `?ok=1`
6. The next headscale push (immediate on ApplyACLPipelineForPlane) makes
   msi's `tag:dev-skyadmin-msi` packets to autogroup:internet use
   karolina as the exit-node

### Pin a different device (e.g. skyworker → karolina)
