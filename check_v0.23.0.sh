...
#   4. DB: portal_users.headscale_url for admin is set
#   5. DB: portal_users.headscale_api_key_enc is non-empty (encrypted)
#   6. per-user headscale container is up + healthy
#   7. per-user headscale has the user "admin" in its DB
#   8. headscale-admin listens on port 50451 (admin uid=1)
...
# was created manually via `bash headscale-bootstrap.sh admin 1`
...
if echo "$out" | grep -qE 'value="http://headscale-admin:'; then
...
if echo "$out" | grep -q "headscale-admin" 2>/dev/null; then
...
if echo "$out2" | grep -qE 'http://headscale-admin'; then
...
echo
echo "--- 4. DB: portal_users.headscale_url for admin is set"
db_url=$(docker exec skygate sqlite3 /data/skygate.db \
    "SELECT headscale_url FROM portal_users WHERE username='admin'" 2>&1)
if [ "$db_url" = "http://headscale-admin:50451" ]; then
...
if [ -n "$db_url" ] && [ "$db_url" != "" ]; then
    db_key_enc=$(docker exec skygate sqlite3 /data/skygate.db \
        "SELECT headscale_api_key_enc FROM portal_users WHERE username='admin'" 2>&1)
...
if docker ps --format '{{.Names}}\t{{.Status}}' | grep -q "^headscale-admin.*healthy"; then
    echo "PASS: headscale-admin is up + healthy"
else
    echo "FAIL: headscale-admin is not running / not healthy"
    docker ps -a --format '{{.Names}}\t{{.Status}}' | grep headscale-admin
    exit 1
fi

echo
echo "--- 7. per-user headscale has the user 'admin' in its DB"
if docker exec headscale-admin /ko-app/headscale users list 2>&1 | grep -q "admin"; then
    echo "PASS: per-user headscale has user 'admin'"
else
    echo "FAIL: per-user headscale is missing user 'admin'"
    exit 1
fi
...
# the same network, you reach it via http://headscale-admin:50451.
if curl -sf http://localhost:50451/api/v1/node 2>&1 | grep -qE '"nodes":\s*\['; then
    echo "PASS: headscale-admin:50451 is reachable from host (and from any container on the headscale_default network via the same hostname:port)"
else
    ...
    if curl -sf http://headscale-admin:50451/api/v1/node 2>&1 | grep -qE '"nodes":\s*\['; then
        echo "PASS: headscale-admin:50451 reachable via DNS hostname (docker network)"
    else
        echo "INFO: headscale-admin:50451 is on the docker network but the host can't reach it directly (expected: only containers on headscale_default network can reach it). The fact that skygate's HSForUser(1) can talk to it is verified via the form pre-fill + DB write path above."
    fi
fi
...
if echo "$out3" | grep -q 'value="http://headscale-admin:50451"'; then
...
if grep -q "control_planes.provisioned" /home/admin/skygate/internal/i18n/catalog.go; then
