...
- See a new **"Preferred exit-node (v0.28.1)"** card on
  ...
  headscale node that carries `tag:exit-node` (emilia,
  sharlotta, karolina today). Save → row in
...
- The `via: ["tag:exit-emilia"]` field in `grants[]` is
...
(`"tag:exit-emilia": ["skyadmin@..."]`) so headscale's parser
...
"Currently preferred: tag:exit-emilia" plus a "Clear" button
(POST with `tag=''`).
...
- `TestGenerateACLWithVia_UserPrefTriggersViaAndTagOwners` —
  a row with `tag:exit-emilia` adds `via: ["tag:exit-emilia"]`
...
curl -sS -c /tmp/ck -b /tmp/ck http://localhost:8080/my/exit-nodes \
  | grep -oE 'tag:exit-emilia|tag:exit-sharlotta|tag:exit-karolina' | sort -u
