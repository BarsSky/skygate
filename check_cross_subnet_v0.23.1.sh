...
#       admin and at least one other user (v0.22.0).
...
#   9.  The per-user headscale-admin container is up
#       (from v0.23.0 Phase 1) but is NOT in the live
#       routing path for admin's actual traffic (because
...
# (user1, guest, user2 — all created in 2026-07-16) have
# no row. The script reports each user's subnet state and
# confirms that at least admin (post-v0.20.0) has one.
...
echo "$rows"
admin_cidr=$(echo "$rows" | awk -F'|' '$1=="admin" {print $2}')
admin_status=$(echo "$rows" | awk -F'|' '$1=="admin" {print $3}')
if [ "$admin_cidr" = "10.0.1.0/24" ]; then
    echo "PASS: admin has 10.0.1.0/24 (per-user subnet allocated, v0.16.6+ logic)"
else
    echo "FAIL: admin's subnet = $admin_cidr, want 10.0.1.0/24"
    exit 1
fi
# The status reflects v0.22.3's SyncStatus logic, which counts
# rows in node_owner_map. admin's devices are all in the
# "tagged-devices" namespace (because they have tags), so
# node_owner_map is empty for admin, and the status is
...
# the architecture.
echo "    status=$admin_status (v0.22.3 spec: pending ⇔ 0 rows in node_owner_map;"
echo "    admin's devices are in 'tagged-devices' namespace due to v0.3.9 backfill"
echo "    limitations — separate concern from cross-subnet architecture)"
# Pre-v0.20.0 users have no row — that's expected (documented
# in v0.22.3 release notes: "user1/guest/user2 were
# pre-v0.20.0, no subnet row — stays '—' in UI").
echo
echo "Note: pre-v0.20.0 users (user1/guest/user2) have no subnet"
...
# Look for per-user rules like 10.0.1.0/24 (admin).
...
if echo "$out" | grep -q "10.0.1.0/24"; then
    echo "PASS: /my/devices shows admin's CIDR 10.0.1.0/24"
else
    echo "FAIL: /my/devices does NOT show admin's CIDR"
    exit 1
fi
...
# After v0.23.0, admin has its own plane URL in the DB.
# The landing should show at least 2 rows: the global default
# (used by user1/guest/user2) + the admin per-user one.
...
if grep -q "control_planes.provision_warning_title" /home/admin/skygate/internal/i18n/catalog.go; then
...
echo "  - Exit-nodes (relay-1/relay-2/relay-3) are shared across all users via tag:exit-node ACL."
