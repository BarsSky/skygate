...
#                    — SSH to the relay (RELAY=relay-1) and re-derive
...
# into the VM (SSH_HOST env var, default admin@192.0.2.1)
...
# SSH_HOST (default admin@192.0.2.1) with the
...
tailscale-update-telegram-routes:
	@if [ -z "$(RELAY)" ]; then \
		echo "RELAY=<hostname> required, e.g. make tailscale-update-telegram-routes RELAY=relay-1"; \
		exit 1; \
	fi
