...
└─────────────┘     └──────┬───────┘     └────────┬─────────┘
                    ...
                    │              │     │  relay-1           │
                    │  Clients:    │     │  relay-2        │
                    │  workstation-1   │     │  --advertise-     │
                    │  workstation-2     │     │    exit-node      │
                    │  ...         │     │  --advertise-     │
                    └──────────────┘     │    routes         │
                                         └──────────────────┘
```
...
| `SKYGATE_ADMIN_USER` | Имя админа | `admin` |
| `SKYGATE_ADMIN_PASS` | Пароль админа | `...` |
| `SKYGATE_CONTROL_URL` | Публичный URL headscale | `https://head.example.com` |
| `HEADSCALE_CONTAINER` | Имя Docker-контейнера headscale | `headscale` |
| `SKYGATE_EXIT_SSH_KEY` | Путь к SSH-ключу для автосинка | `/home/admin/.ssh/skygate_sync` |
| `SKYGATE_EXIT_SSH` | SSH-цель для автосинка (все ноды) | `root@100.64.100.2` |
| `SKYGATE_EXIT_SSH_KAROLINA` | SSH на конкретную ноду | `root@relay-3.local` |

## Headscale Config

Файл: `/etc/headscale/config.yaml` (в контейнере headscale), хост-путь: `/home/admin/headscale/config/config.yaml`
...
Выполнить на КАЖДОЙ exit-node (relay-3, relay-1, relay-2):

```bash
# Скачать скрипт:
# https://skygate.example.com/admin/exit-rules → "Sync Advertised Routes" → команда
...
- SSH-правила: `tag:private + admin → tag:exit-node` (root)
- TagOwners, Groups
...
2. На agent VM: положить ключ `/home/admin/.ssh/skygate_sync`
3. В `/tmp/skygate.env`: `SKYGATE_EXIT_SSH_KEY=/home/admin/.ssh/skygate_sync`
...
CREATE TABLE device_rules (
    ...
    exit_node_id TEXT NOT NULL,    -- имя exit-node (relay-3/relay-1/relay-2)
...
docker run --rm -v /home/admin/skygate:/app -v /tmp:/host alpine cp /host/file.go /app/internal/handlers/target.go
```
