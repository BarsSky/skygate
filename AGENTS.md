...
```json
{
  ...
  "tagOwners": {
    ...
    "tag:exit-node": ["admin@tsnet.example.com"]
  },
...
tailnet (a "relay", e.g. relay-1) advertises the canonical Telegram IP
...
* `deploy/tailscale-relay/setup.sh` — run on the relay host
  (relay-1/relay-2/relay-3). Calls `tailscale set
...
Both **relay-1** and **relay-2** advertise the same 8 v4 + 4 v6
Telegram CIDRs; Tailscale picks one based on metric. If relay-1
goes down, the bot automatically starts using relay-2 within
~60s. Both have a weekly cron (`0 4 * * 1`) running
`/usr/local/bin/skygate-update-telegram-routes` to refresh the
routes from DNS. **relay-3** is available as a third-tier backup
