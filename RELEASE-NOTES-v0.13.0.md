...
* `online → offline`  — "🛰️ exit-node relay-1: online → offline (went offline, 2026-07-15 14:23Z)"
* `offline → online`  — "🛰️ exit-node relay-1: offline → online (came back online, 2026-07-15 14:30Z)"
...
Online (2):
• relay-1           online    last_seen: 1m ago    last_check: 2026-07-15 14:32Z
• relay-2        online    last_seen: 1m ago    last_check: 2026-07-15 14:32Z

Offline (1):
• relay-3         offline   last_seen: 2h ago    last_check: 2026-07-15 14:32Z
```
...
1. `docker exec headscale headscale nodes tag -i 11 -r tag:exit-node`
   (removes the tag from relay-3, id=11).
2. Wait one monitor tick (or click "Run health check now").
3. Telegram should receive:
   `🛰️ exit-node relay-3: online → degraded (tag:exit-node removed, 2026-07-15 14:35Z)`.
   Note: degraded transitions are recorded in the audit
   log but NOT alerted (calm mode).

To simulate an offline exit-node:

1. `tailscale down` on relay-1 (or power it off).
2. Wait until `last_seen` is older than 2 min.
3. `🛰️ exit-node relay-1: online → offline (went offline, ...)` is sent.
4. When relay-1 comes back, `🛰️ exit-node relay-1: offline → online (came back online, ...)` is sent.
