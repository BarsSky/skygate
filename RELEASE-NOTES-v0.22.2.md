...
1. **`backfillNodeOwnership` Strategy A fix** —
   ...
   For nodes that ALREADY have tags (e.g. `skygate-host-1`
...
(id=98, user_id=1=admin, used=1), headscale stored
...
- **Existing nodes with `tag=tag:private` are
  ...
  (e.g. skygate-host-1, which gets it from the v0.16.7
...
scp check_v0.22.2.sh admin@<vm>:/tmp/
ssh admin@<vm> "chmod +x /tmp/check_v0.22.2.sh && bash /tmp/check_v0.22.2.sh"

# 3. Check that MSI (or any freshly-registered device) now has tag:private
ssh admin@<vm> "docker exec headscale headscale nodes list 2>/dev/null | grep -A1 ' MSI'"
# Should show: tag:private

# 4. Check the skygate snapshot
ssh admin@<vm> "docker exec skygate sqlite3 /data/skygate.db \
  'SELECT node_id, username, tag FROM node_owner_map WHERE node_id = 15;'"
# Should show: 15|admin|tag:private
```
