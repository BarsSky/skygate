...
// UpdateNodeOwnerHostnameAndTag rewrites BOTH the hostname AND the
// tag for an existing node_owner_map row, keyed on node_id. The
...
// calls hs.UntagNode(oldTag) so headscale drops the stale tag.
//
// Returns ErrNodeOwnerNotFound if no row exists for nodeID —
// a no-op in practice (backfill only runs after INSERT OR
// IGNORE confirms a row exists), but kept consistent with
// UpdateNodeOwnerTag's error semantics.
func UpdateNodeOwnerHostnameAndTag(d dbExec, nodeID, newHostname, newTag string, taggedByUserID int64) error {
	res, err := d.Exec(
		`UPDATE node_owner_map
		    SET hostname = $1, tag = $2, tagged_by_user_id = $3, tagged_at = strftime('%s','now')
		  WHERE node_id = $4`,
		newHostname, newTag, taggedByUserID, nodeID,
	)
	if err != nil {
		return err
	}
...
func RecoverOwnerUsernameFromPreauth(d *sql.DB, headscalePreauthID string) (string, error) {
	...
	err := d.QueryRow(
		`SELECT pu.username
		  ...
		  WHERE pk.headscale_preauth_id = $1
...
func SetDeviceMetaNodeOwner(d *sql.DB, nodeID, os, deviceType string) error {
	_, err := d.Exec(
		`UPDATE node_owner_map SET os = $1, device_type = $2 WHERE node_id = $3`,
...
func SetDeviceMetaByHostname(d *sql.DB, headscaleUserID int64, hostname, os, deviceType string) error {
	res, err := d.Exec(
		`UPDATE node_owner_map SET os = $1, device_type = $2
		   WHERE headscale_user_id = $3 AND hostname = $4`,
