...
func TestAppendAuditLog(t *testing.T) {
	...
	if err := AppendAuditLog(d, 1, "admin", "user_create", "created alice"); err != nil {
...
func TestDeleteAuditLogByUserID(t *testing.T) {
	...
	if err := AppendAuditLog(d, 1, "admin", "user_create", "x"); err != nil {
		t.Fatalf("append 1: %v", err)
	}
	if err := AppendAuditLog(d, 1, "admin", "rule_add", "y"); err != nil {
		t.Fatalf("append 1b: %v", err)
	}
	if err := AppendAuditLog(d, 2, "user1", "login", "z"); err != nil {
	...
	if left != 1 {
		t.Errorf("rows after delete = %d, want 1 (only user1's)", left)
	}
...
func TestListAuditActionsDistinct(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		if err := AppendAuditLog(d, 1, "admin", "login", ""); err != nil {
			t.Fatalf("append: %v", err)
		}
	}
	if err := AppendAuditLog(d, 1, "admin", "logout", ""); err != nil {
...
//
// 2026-08-10: v0.33.1.41 #2 — V054 auto-creates the 'infra'
// portal_user row at a reserved id (see migrations_v0.54.go: id=99)
// so it doesn't collide with user_ids 1..N that the tests use.
// The helper stores user_id verbatim in device_rules, and the
// LEFT JOIN onto portal_users resolves the username from the
// test's seeded "userN" row (id=N) — V054's infra row is
...
func TestAppendAndFindDeviceRule(t *testing.T) {
	...
	id := insertRule(t, d, 1, 8, "relay-3", "domain", "example.com", "accept", "100.64.100.1", "example.com")
	if id <= 0 {
		t.Fatalf("AppendDeviceRule returned id=%d, want > 0", id)
	}

	// FindDeviceRuleID should locate the just-inserted row.
	gotID, err := FindDeviceRuleID(d, 1, 8, "relay-3", "domain", "example.com")
	if err != nil {
...
func TestFindDeviceRuleID_NotFound(t *testing.T) {
	...
	_, err := FindDeviceRuleID(d, 1, 8, "relay-3", "domain", "nope.com")
...
func TestFindDomainRuleID(t *testing.T) {
	...
	insertRule(t, d, 1, 8, "relay-3", "domain", "example.com", "accept", "", "example.com")

	// Same (user, device, exit_node, domain) tuple, different target_type
	// (e.g. an IP) — FindDomainRuleID should NOT match it.
	insertRule(t, d, 1, 8, "relay-3", "ip", "1.2.3.4", "accept", "", "")

	gotID, err := FindDomainRuleID(d, 1, 8, "relay-3", "example.com")
	if err != nil {
	...
	_, err = FindDomainRuleID(d, 1, 8, "relay-3", "other.com")
	if !errors.Is(err, ErrNotFound) {
...
func TestCountEnabledRules(t *testing.T) {
	...
	insertRule(t, d, 1, 8, "relay-3", "domain", "a.com", "accept", "", "a.com")
	insertRule(t, d, 1, 9, "relay-3", "domain", "b.com", "accept", "", "b.com")
	insertRule(t, d, 2, 6, "relay-1", "ip", "1.1.1.1", "accept", "", "")
...
func TestCountEnabledRulesForDevice(t *testing.T) {
	...
	insertRule(t, d, 1, 8, "relay-3", "domain", "a.com", "accept", "", "a.com")
	insertRule(t, d, 1, 8, "relay-3", "ip", "1.1.1.1", "accept", "", "")
	insertRule(t, d, 1, 9, "relay-3", "domain", "b.com", "accept", "", "b.com")
...
func TestCountEnabledNonSubnetRulesForUser(t *testing.T) {
	...
	insertRule(t, d, 1, 8, "relay-3", "subnet", "1.2.3.4/32", "accept", "", "a.com")
	// Subnet /32 WITHOUT parent_domain → counts (orphaned /32, e.g. legacy).
	insertRule(t, d, 1, 8, "relay-3", "subnet", "5.6.7.8/32", "accept", "", "")
	// IP rule → counts.
	insertRule(t, d, 1, 8, "relay-3", "ip", "9.10.11.12", "accept", "", "")
...
func TestCountRulesByDeviceForUser(t *testing.T) {
	...
	insertRule(t, d, 1, 9, "relay-3", "subnet", "1.2.3.4/32", "accept", "", "b.com")
...
func TestGetDeviceRulesForUser(t *testing.T) {
	...
	r := rules[0]
	if r.UserID != 1 || r.DeviceID != 8 || r.ExitNodeID != "relay-3" ||
		r.TargetType != "domain" || r.TargetValue != "a.com" || !r.Enabled {
		t.Errorf("rule = %+v, want user=1 device=8 exit=relay-3 domain a.com enabled", r)
	}
}
...
func TestGetAllRulesForAdmin(t *testing.T) {
	...
	// (id=N, username="userN"). The test checks the user_id-keyed
	// map of usernames. V054's infra row sits at id=99 and isn't
	// joined (no device_rule points at id=99), so it doesn't
	// appear in `got` — only the seeded "user1" / "user2" do.
	got := map[int]string{}
	for _, r := range rules {
...
func TestGetACLEntries(t *testing.T) {
	...
	insertRule(t, d, 1, 8, "relay-3", "domain", "a.com", "accept", "100.64.100.1", "a.com")
	insertRule(t, d, 1, 8, "relay-3", "ip", "1.1.1.1", "deny", "100.64.100.1", "")
	...
	// First row: domain a.com, action accept, device_ip 100.64.100.1.
	if entries[0].TargetType != "domain" || entries[0].Action != "accept" || entries[0].DeviceIP != "100.64.100.1" {
...
func TestGetEnabledSubnetIPRules(t *testing.T) {
	...
	insertRule(t, d, 1, 8, "relay-3", "subnet", "10.0.0.0/8", "accept", "", "")
	insertRule(t, d, 1, 8, "relay-3", "ip", "1.1.1.1", "accept", "", "")
	insertRule(t, d, 1, 8, "relay-3", "domain", "a.com", "accept", "", "a.com") // excluded
...
func TestListDistinctExitNodesWithRules(t *testing.T) {
	...
	// Two rules on relay-3, one on relay-1 — distinct should be 2.
	insertRule(t, d, 1, 8, "relay-3", "domain", "a.com", "accept", "", "a.com")
	insertRule(t, d, 1, 8, "relay-3", "ip", "1.1.1.1", "accept", "", "")
	insertRule(t, d, 1, 9, "relay-1", "ip", "2.2.2.2", "accept", "", "")
	...
	sort.Strings(list)
	if len(list) != 2 || list[0] != "relay-1" || list[1] != "relay-3" {
		t.Errorf("got %v, want [relay-1 relay-3]", list)
	}
}
...
func TestDeleteRuleAndDeleteRuleForUser(t *testing.T) {
	...
	id1 := insertRule(t, d, 1, 8, "relay-3", "domain", "a.com", "accept", "", "a.com")
	id2 := insertRule(t, d, 2, 6, "relay-1", "ip", "1.1.1.1", "accept", "", "")
...
func TestDeleteRuleOrCascadeByParentDomain(t *testing.T) {
	...
	insertRule(t, d, 1, 8, "relay-3", "domain", "a.com", "accept", "", "a.com")
	id32a := insertRule(t, d, 1, 8, "relay-3", "subnet", "1.1.1.1/32", "accept", "", "a.com")
	id32b := insertRule(t, d, 1, 8, "relay-3", "subnet", "1.1.1.2/32", "accept", "", "a.com")
	// Unrelated /32 with a different parent — must NOT be removed.
	insertRule(t, d, 1, 8, "relay-3", "subnet", "2.2.2.2/32", "accept", "", "b.com")
