...
func migrate(d *sql.DB) error {
	...
	// 2026-08-09: v0.33.1.19 — via_enabled column repair.
	// Walks user_exit_node_prefs and device_exit_node_prefs
	// and swaps the via_enabled / updated_at values that
	// were inserted in the wrong order by the v0.28.5 INSERT
	// bug. See migrations_v0.52.go for the full rationale.
	if err := migrateV052(d); err != nil {
		return fmt.Errorf("migrate v0.52: %w", err)
	}
	// 2026-08-10: v0.33.1.33 — exit_servers.ssh_port column.
	// Per-row SSH port override for the B85 chain. The
	// LookupExitServerSSHTarget helper now reads this column
	// and appends ":<port>" to the B81 auto-fallback
	// ("root@<tailscale_ip>:<port>") when set. Empty = port
	// 22 (the v0.33.1.29 behaviour, preserved for backward
	// compat). See migrations_v0.53.go for the full rationale.
	if err := migrateV053(d); err != nil {
		return fmt.Errorf("migrate v0.53: %w", err)
	}
	// 2026-08-10: v0.33.1.39 — Issue 4 technical user. Creates
	// the 'infra' portal_user row (a system account for
	// skygate-host-* + exit-node + subnet-router devices).
	// The corresponding headscale user is provisioned at
	// startup by ensureInfraUser in cmd/skygate/main.go.
	// See migrations_v0.54.go for the full rationale.
	if err := migrateV054(d); err != nil {
		return fmt.Errorf("migrate v0.54: %w", err)
	}
	return nil
}
