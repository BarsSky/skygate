...
func TestMigrateV047_SecondRun_PreservesViaZero(t *testing.T) {
	...
	if _, err := d.Exec("INSERT INTO user_exit_node_prefs (user_id, exit_node_tag, via_enabled) VALUES (1, 'tag:exit-emilia', 0)"); err != nil {
		t.Fatalf("insert user pref: %v", err)
	}
	if _, err := d.Exec("INSERT INTO device_exit_node_prefs (user_id, device_hostname, exit_node_tag, via_enabled) VALUES (1, 'msi', 'tag:exit-karolina', 0)"); err != nil {
	...
	if err := d.QueryRow("SELECT via_enabled FROM device_exit_node_prefs WHERE user_id = 1 AND device_hostname = 'msi'").Scan(&dev); err != nil {
...
func TestMigrateV047_FirstRun_BackfillsExistingRows(t *testing.T) {
	...
	d.Exec(`INSERT INTO user_exit_node_prefs (user_id, exit_node_tag) VALUES (1, 'tag:exit-emilia')`)
