...
func TestListExitNodeHealth_OrderedByHostname(t *testing.T) {
	d := openTestDB(t)
	now := time.Now().UTC()
	seedExitNodeHealth(t, d, ExitNodeHealth{NodeID: "4", Hostname: "relay-2", State: "online", LastCheckAt: now})
	seedExitNodeHealth(t, d, ExitNodeHealth{NodeID: "3", Hostname: "relay-1", State: "online", LastCheckAt: now})
	seedExitNodeHealth(t, d, ExitNodeHealth{NodeID: "11", Hostname: "relay-3", State: "offline", LastCheckAt: now})
...
func TestCountHealthyExitNodes_OnlyCountsHealthy(t *testing.T) {
	d := openTestDB(t)
	now := time.Now().UTC()
	seedExitNodeHealth(t, d, ExitNodeHealth{NodeID: "3", Hostname: "relay-1", State: "online", Healthy: true, LastCheckAt: now})
	seedExitNodeHealth(t, d, ExitNodeHealth{NodeID: "4", Hostname: "relay-2", State: "offline", Healthy: false, LastCheckAt: now})
	seedExitNodeHealth(t, d, ExitNodeHealth{NodeID: "11", Hostname: "relay-3", State: "degraded", Healthy: false, LastCheckAt: now})
	...
	if got != 1 {
		t.Errorf("CountHealthyExitNodes = %d, want 1 (only relay-1)", got)
	}
}
...
func TestDeleteExitNodeHealth_OnlyRemovesSnapshot(t *testing.T) {
	...
	now := time.Now().UTC()
	seedExitNodeHealth(t, d, ExitNodeHealth{
		NodeID: "3", Hostname: "relay-1", State: "online",
