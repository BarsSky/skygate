...
func TestLastSyncForExitNode(t *testing.T) {
	...
	// Empty DB → no sync → 0
	ts, err := LastSyncForExitNode(d, "relay-3")
	if err != nil {
	...
	// Add a sync log line mentioning relay-3
	if err := AppendExitRuleLog(d, 1, ExitRuleActionSync, "sync relay-3: ok"); err != nil {
		t.Fatalf("append: %v", err)
	}
	// And one for a different node
	if err := AppendExitRuleLog(d, 1, ExitRuleActionSync, "sync relay-1: ok"); err != nil {
		t.Fatalf("append: %v", err)
	}
	...
	ts2, err := LastSyncForExitNode(d, "relay-1")
	if err != nil {
		t.Fatalf("relay-1: %v", err)
	}
	if ts2 == 0 {
		t.Errorf("relay-1 ts = 0, want > 0")
	}

	// Non-existent node → 0
	ts3, err := LastSyncForExitNode(d, "relay-2")
	if err != nil {
		t.Fatalf("relay-2: %v", err)
	}
	if ts3 != 0 {
		t.Errorf("relay-2 ts = %d, want 0", ts3)
	}
}
...
func TestNextACLVersion(t *testing.T) {
	...
	if err := SaveACLSnapshot(d, 1, "{}", "admin"); err != nil {
	...
	if err := SaveACLSnapshot(d, 5, "{}", "admin"); err != nil {
...
func TestMarkACLAppliedAndFail(t *testing.T) {
	...
	if err := SaveACLSnapshot(d, 1, `{"acls":[]}`, "admin"); err != nil {
		t.Fatalf("SaveACLSnapshot: %v", err)
	}
	...
	if err := SaveACLSnapshot(d, 2, `{"acls":[]}`, "admin"); err != nil {
