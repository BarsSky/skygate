...
func TestSetAndGetDefaultExitNode(t *testing.T) {
	...
	if n, err := SetDefaultExitNode(d, id, "relay-3-node-7"); err != nil || n != 1 {
		t.Fatalf("SetDefaultExitNode n=%d err=%v", n, err)
	}
	got, _ := GetDefaultExitNode(d, id)
	if got != "relay-3-node-7" {
		t.Errorf("after set, default exit_node = %q, want %q", got, "relay-3-node-7")
	}
...
func TestDefaultDeviceAndExitNodeIndependent(t *testing.T) {
	...
	if _, err := SetDefaultExitNode(d, id, "relay-1-1"); err != nil {
		t.Fatalf("SetDefaultExitNode: %v", err)
	}

	// device still 1, exit_node now relay-1-1
	gotDev, _ := GetDefaultDevice(d, id)
	gotExit, _ = GetDefaultExitNode(d, id)
	if gotDev != "device-1" {
		t.Errorf("after SetDefaultExitNode, device = %q, want %q", gotDev, "device-1")
	}
	if gotExit != "relay-1-1" {
		t.Errorf("after SetDefaultExitNode, exit_node = %q, want %q", gotExit, "relay-1-1")
	}
}
