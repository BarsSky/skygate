...
func SetDeviceExitNodePref(d *sql.DB, userID int64, deviceHostname, exitNodeTag string, setByUserID int64, viaEnabled bool) error {
	...
	//
	// Same bug as the user_exit_node_prefs INSERT (see the
	// comment in migrations_v0.45.go SetUserExitNodePref):
	// VALUES listed 5 placeholders + 1 nowUnixSQL in the
	// order (userID, deviceHostname, exitNodeTag, setByUserID,
	// viaInt, nowUnixSQL). Columns were (user_id,
	// device_hostname, exit_node_tag, set_by_user_id,
	// updated_at, via_enabled). viaInt went into updated_at
	// and nowUnixSQL into via_enabled — every row had
	// updated_at=0/1 and via_enabled=<unix timestamp>,
	// which is always truthy, so the per-device grant in
	// the ACL was always emitted with via=tag:exit-... and
	// the per-device "via enabled" checkbox on /my/devices
	// was a no-op.
	//
	// 2026-08-09 v0.33.1.27 — placeholdersRange + drop
	// nowUnixSQL() from the Go arg list.
	//
	// Same root cause as the user_exit_node_prefs INSERT —
	...
	// matching 5 Go args.
	//
	// Schema (unchanged): user_id, device_hostname,
	// exit_node_tag, set_by_user_id, updated_at, via_enabled
	// Values (corrected):  userID, deviceHostname, exitNodeTag,
	//                       setByUserID, nowUnixSQL (SQL fn),
	//                       viaInt
	_, err := d.Exec(`
