...
func SetUserExitNodePref(d *sql.DB, userID int64, exitNodeTag string, setByUserID int64, viaEnabled bool) error {
	...
	// 2026-08-09 v0.33.1.19 — INSERT column order fix.
	//
	// Pre-fix the VALUES clause listed 4 placeholders + 1
	// nowUnixSQL() in the order (userID, exitNodeTag,
	// setByUserID, viaInt, nowUnixSQL). The column list was
	// (user_id, exit_node_tag, set_by_user_id, updated_at,
	// via_enabled). Positional mismatch: viaInt (a 0/1 bool)
	// was written into updated_at, and nowUnixSQL() (a
	// unix timestamp > 1.7e9) was written into via_enabled.
	// Result: every row in user_exit_node_prefs had
	// updated_at=0 or 1, via_enabled=<unix timestamp> — and
	// "via_enabled != 0" is always true for a unix timestamp,
	// so the ACL builder always emitted `via: [tag:exit-...]`
	// in the per-user grant (no way to opt out via the UI
	// checkbox) and the /my/exit-nodes page always showed
	// the "strict" badge regardless of operator choice.
	//
	// 2026-08-09 v0.33.1.27 — placeholdersRange + drop
	// nowUnixSQL() from the Go arg list.
	//
	// The v0.33.1.19 fix put `placeholdersList(3),
	// placeholdersList(1)` in the SQL — on PG this produced
	// "$1, $2, $3, $1" (TWO references to $1 in the same
	// query, because placeholdersList always starts from 1).
	// pgx rejected the query with
	...
	//
	// Schema (unchanged): user_id, exit_node_tag,
	// set_by_user_id, updated_at, via_enabled
	// Values (corrected):  userID, exitNodeTag, setByUserID,
	//                       nowUnixSQL (SQL fn, spliced),
	//                       viaInt
	_, err := d.Exec(`
