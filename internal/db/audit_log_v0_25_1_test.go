...
func TestListAuditLogForUser(t *testing.T) {
	...
	// Seed: 3 rows for user_id=1, username=admin;
	// 2 rows for user_id=0, username=admin (system
	// events on the user's behalf); 1 row for user_id=2,
	// username=user1 (a different user — must NOT
	// appear in admin's export).
	if err := AppendAuditLog(d, 1, "admin", "login_ok", ""); err != nil {
		t.Fatalf("AppendAuditLog: %v", err)
	}
	if err := AppendAuditLog(d, 1, "admin", "subnet_provision", "user_id=1"); err != nil {
		t.Fatalf("AppendAuditLog: %v", err)
	}
	if err := AppendAuditLog(d, 1, "admin", "preauth_issued", "1h single-use"); err != nil {
		t.Fatalf("AppendAuditLog: %v", err)
	}
	if err := AppendAuditLogNoUser(d, "admin", "telegram_restart", "by user 1"); err != nil {
		t.Fatalf("AppendAuditLogNoUser: %v", err)
	}
	if err := AppendAuditLogNoUser(d, "admin", "telegram_ack", "alert_id=42"); err != nil {
		t.Fatalf("AppendAuditLogNoUser: %v", err)
	}
	if err := AppendAuditLog(d, 2, "user1", "login_ok", ""); err != nil {
		t.Fatalf("AppendAuditLog: %v", err)
	}

	// Export for admin (user_id=1, username=admin)
	// — should return ALL 5 rows (3 user-owned + 2 system).
	rows, err := ListAuditLogForUser(d, 1, "admin", 0, 100, 0)
	if err != nil {
	...
	for _, r := range rows {
		if r.Username != "admin" {
			t.Errorf("row username = %q, want admin (no cross-user leak)", r.Username)
		}
	}

	// Export for user1 — only 1 row.
	rows2, err := ListAuditLogForUser(d, 2, "user1", 0, 100, 0)
	if err != nil {
		t.Fatalf("ListAuditLogForUser (user1): %v", err)
	}
	if len(rows2) != 1 {
		t.Errorf("len(rows) for user1 = %d, want 1", len(rows2))
	}

	// limit=2 — only 2 rows.
	rows3, err := ListAuditLogForUser(d, 1, "admin", 0, 2, 0)
	if err != nil {
	...
	rows4, err := ListAuditLogForUser(d, 1, "admin", 0, 100, 3)
	if err != nil {
	...
	rows5, _ := ListAuditLogForUser(d, 1, "admin", now+3600, 100, 0)
	if len(rows5) != 0 {
		t.Errorf("rows with future since = %d, want 0", len(rows5))
	}
	rows6, _ := ListAuditLogForUser(d, 1, "admin", now-3600, 100, 0)
	if len(rows6) != 5 {
