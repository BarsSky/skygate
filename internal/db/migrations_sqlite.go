...
func migrateV020SQLite(d *sql.DB) error {
	...
	if err := execSQLiteDDL(d, queries); err != nil {
		return err
	}
	return nil
}
...
func migrateV068SQLite(d *sql.DB) error {
	...
	// execSQLiteDDL wraps the failing statement in the error (and treats
	// ADD COLUMN idempotently), so the per-statement %q formatting that used
	// to live here is no longer needed.
	if err := execSQLiteDDL(d, stmts); err != nil {
		return fmt.Errorf("v0.68 (B232): %w", err)
	}
	return nil
}
