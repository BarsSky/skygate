...
func TestBackfill_StrategyA_TaglessNode_InsertsRow(t *testing.T) {
	...
	// Seed: portal user skyadmin with headscale_user_id=1
	if _, err := d.Exec(
	...
	node := headscale.NodeView{
		ID:          "15",
		Hostname:    "msi",
		UserID:      "1", // in skyadmin's namespace
		PreAuthKeyID: "98",
	...
	if n.Username != "skyadmin" {
		t.Errorf("username = %q, want skyadmin", n.Username)
	}
	if n.Tag != "tag:private" {
		t.Errorf("tag = %q, want tag:private (v0.22.2 fix)", n.Tag)
	}
	if n.Hostname != "msi" {
		t.Errorf("hostname = %q, want msi", n.Hostname)
	}
}
...
func TestBackfill_NonMatchingPreauth_NoInsert(t *testing.T) {
	...
	// key (someone else's key, e.g. michail's).
	node := headscale.NodeView{
		ID:          "20",
		Hostname:    "stranger",
		UserID:      "1",
...
// the dashboard forever (the original michail "0/0"
// report symptom).
func TestBackfill_GCPassRemovesOrphanRows(t *testing.T) {
	...
	if err := db.UpsertNodeOwner(d, "100", 1, "skyadmin", "tag:private", 1); err != nil {
		t.Fatalf("seed snapshot: %v", err)
	}
	// Live node list: empty (the "100" node is gone).
	Backfill(d, nil, nil, 1, "skyadmin")
