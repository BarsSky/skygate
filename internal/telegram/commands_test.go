...
func TestMyRulesReplyUserFiltersToCaller(t *testing.T) {
	...
	// The seed for admin uses target_value "x" — alice must not see those.
	if strings.Contains(got, "\n  domain x →") {
...
func TestMyExitNodesReplyListsEnabled(t *testing.T) {
	...
	if !strings.Contains(got, "relay-1") || !strings.Contains(got, "online") {
		t.Errorf("expected relay-1 marked online, got: %q", got)
	}
...
func TestMyExitNodesReplyMarksDefault(t *testing.T) {
	...
	// relay-1 must NOT carry the [default] marker.
	if strings.Contains(got, "relay-1-1) — offline  [default]") {
		t.Errorf("relay-1 must not be marked default, got: %q", got)
	}
}
