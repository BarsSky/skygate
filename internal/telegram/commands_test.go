...
func setupTestDB(t *testing.T) *sql.DB {
	...
	for _, q := range []string{
		...
		// 2026-08-10: v0.33.1.33 B85 — added ssh_port (the per-row
		// non-default SSH port for the B81 auto-fallback). Mirrors
		// the production schema from migrations_v0.24 + V053.
		// Without this column, the test schema's ListExitServers
		// returns "no such column: ssh_port" on every telegram
		// command that reads the exit-nodes list.
		`CREATE TABLE exit_servers (id INTEGER PRIMARY KEY AUTOINCREMENT, node_id TEXT NOT NULL UNIQUE, hostname TEXT NOT NULL, tailscale_ip TEXT NOT NULL DEFAULT '', ssh_target TEXT NOT NULL DEFAULT '', ssh_key_path TEXT NOT NULL DEFAULT '', ssh_port TEXT NOT NULL DEFAULT '', description TEXT DEFAULT '', enabled INTEGER NOT NULL DEFAULT 1, accept_routes INTEGER NOT NULL DEFAULT 0, created_at INTEGER DEFAULT 0)`,
	...
	// rows are owned by admin (user_id=1) so /quota sees the
	// expected 12-rule count under that user; /rules is the only
	// command that doesn't care about user_id.
	_, _ = d.Exec(`INSERT INTO portal_users(id, username, is_admin) VALUES (1, 'admin', 1)`)
	...
	_, _ = d.Exec(`INSERT INTO node_owner_map(node_id, username, tag) VALUES ('n1', 'admin', 'tag:private')`)
	_, _ = d.Exec(`INSERT INTO node_owner_map(node_id, username, tag) VALUES ('n2', 'admin', 'tag:private')`)
	_, _ = d.Exec(`INSERT INTO node_owner_map(node_id, username, tag) VALUES ('n3', 'admin', 'tag:public')`)
	_, _ = d.Exec(`INSERT INTO audit_log(username, action, detail, created_at) VALUES ('admin', 'user_create', 'created alice', 1700000000)`)
	_, _ = d.Exec(`INSERT INTO audit_log(username, action, detail, created_at) VALUES ('admin', 'telegram_save', 'token=*** chat=1', 1700000010)`)
	// Phase-3 seeds: a tagged exit-node with a recent last_seen,
	// and one alert row to ack.
	_, _ = d.Exec(`INSERT INTO node_owner_map(node_id, username, tag) VALUES ('exit-relay-1', 'admin', 'tag:exit-node')`)
	_, _ = d.Exec(`INSERT INTO devices(node_id, last_seen, online) VALUES ('exit-relay-1', 1700000200, 1)`)
	_, _ = d.Exec(`INSERT INTO telegram_alerts(body) VALUES ('📥 New rule #7 by admin')`)
	t.Cleanup(func() {
		_ = d.Close()
		_ = filepath.Clean("")
...
func TestHandleCommandExitNodes(t *testing.T) {
	...
	if !strings.Contains(got, "exit-relay-1") {
...
func TestHandleCommandQuota(t *testing.T) {
	d := setupTestDB(t)
	// admin has all 12 rules; alice has 0. With DefaultMax=200,
	// admin should show 12/200 ~ 6%, alice should show 0/200 ~ 0%.
	env := BotEnv{DB: d, DefaultMax: 200, Lang: i18n.LangEN}
	got := HandleCommand(context.Background(), env, "/quota")
	if !strings.Contains(got, "admin") {
		t.Errorf("expected admin in quota, got: %q", got)
	}
...
func TestHandleCommandQuotaPerUserOverride(t *testing.T) {
	d := setupTestDB(t)
	// admin gets a tiny 10-rule cap so it shows as warning-level
	// fill (12/10 → 100%). alice stays at the default.
	env := BotEnv{DB: d, UserMaxRules: map[string]int{"admin": 10}, DefaultMax: 200}
	got := HandleCommand(context.Background(), env, "/quota")
	if !strings.Contains(got, "12") {
		t.Errorf("expected rule count, got: %q", got)
	}
	...
	// the per-user override is scoped to admin only.
...
func adminEnv(d *sql.DB) BotEnv {
	return BotEnv{DB: d, ChatID: 999, PortalUserID: 1, Username: "admin", IsAdmin: true, Lang: i18n.LangEN}
}

func TestMyStatusReplyUser(t *testing.T) {
	d := setupTestDB(t)
	got := myStatusReply(userEnv(d))
	// alice owns 0 rules (only admin's 12 are seeded), and 0 devices.
	if !strings.Contains(got, "alice") {
...
func TestMarkHTMLReplyPreservesKeyboard(t *testing.T) {
	...
	_, _ = d.Exec(`INSERT INTO exit_servers(node_id, hostname, enabled) VALUES ('relay-1-1', 'relay-1', 1)`)
	pendingReplyForCurrentMessage = nil
...
func TestMyNodesReplyUserFiltersToCaller(t *testing.T) {
	...
	// admin's nodes must NOT leak through.
	if strings.Contains(got, "n1") {
		t.Errorf("alice must not see admin's nodes, got: %q", got)
	}
}
...
func TestHostnameMapFromHeadscale_BuildsMapFromRealClient(t *testing.T) {
	_, hs := fakeNodeServer(t, []headscale.HSNode{
		{ID: "1", GivenName: "skygate-host-1", Name: "other"},
	...
	if got["1"] != "skygate-host-1" {
		t.Errorf("id=1: got %q, want skygate-host-1", got["1"])
	}
...
func TestMyRulesReplyUserFiltersToCaller(t *testing.T) {
	d := setupTestDB(t)
	// Seed: alice has 1 rule, admin has 12 (from setup).
	...
	if strings.Contains(got, "  domain x →") {
		t.Errorf("alice must not see admin's rules, got: %q", got)
	}
...
func TestDeleteRuleReplyRejectsCrossUser(t *testing.T) {
	d := setupTestDB(t)
	// Insert a rule owned by admin (id=1). alice (userEnv) tries
	...
	res, _ := d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES (1, 'relay-1', 'domain', 'foo.com', 'accept')`)
	...
	if n != 1 {
		t.Errorf("expected admin's rule to be preserved, got %d rows", n)
	}
}
...
func TestResolveTargetUser(t *testing.T) {
	...
	u, isOther, err = resolveTargetUser(userEnv(d), "admin")
	if err != nil {
		t.Fatalf("other: %v", err)
	}
	if !isOther || u.Username != "admin" {
		t.Errorf("other username should resolve to admin with isOther=true, got user=%+v isOther=%v", u, isOther)
	}
...
func adminEnvWithHS(d *sql.DB, hs *headscale.Client) BotEnv {
	return BotEnv{DB: d, ChatID: 1, Lang: i18n.LangEN, PortalUserID: 1, Username: "admin", IsAdmin: true, HS: hs}
}
...
func TestAddDeviceReplyV0121_PerUserRouting(t *testing.T) {
	...
	// him to plane B. setupTestDB only seeds admin and
...
func TestAddDeviceReplyAdminForOtherUser(t *testing.T) {
	...
	// preauth_keys row must be under alice (id=2), NOT admin (id=1).
	...
	if adminKeys != 0 {
		t.Errorf("expected 0 preauth_keys rows for admin (admin), got %d", adminKeys)
	}
	// audit_log row under alice, not admin.
	...
	if adminAudit != 0 {
		t.Errorf("expected 0 preauth_issued audit rows for admin, got %d", adminAudit)
	}
}
...
func TestAddDeviceReplyRejectsNonAdminForOtherUser(t *testing.T) {
	...
	// key for admin. The IsIdentified + isAdminArg check fires
	// before HS even gets called.
	got := HandleCommand(context.Background(), userEnvWithHS(d, hs), "/add_device admin")
...
func TestSetDefaultDeviceReplyListsDevices(t *testing.T) {
	d := setupTestDB(t)
	// setupTestDB seeds admin (id=1) with two tag:private devices
...
func TestSetDefaultDeviceRejectsOtherUsersNode(t *testing.T) {
	d := setupTestDB(t)
	// alice has one device of her own; admin's n1 is NOT one
...
func TestSetExitNodeReplyListsEnabled(t *testing.T) {
	d := setupTestDB(t)
	_, _ = d.Exec(`INSERT INTO exit_servers(node_id, hostname, enabled) VALUES ('relay-1-1', 'relay-1', 1)`)
	_, _ = d.Exec(`INSERT INTO exit_servers(node_id, hostname, enabled) VALUES ('relay-3-1', 'relay-3', 1)`)
	_, _ = d.Exec(`INSERT INTO exit_servers(node_id, hostname, enabled) VALUES ('disabled-1', 'disabled', 0)`)
	got := HandleCommand(context.Background(), userEnv(d), "/setexitnode")
	if !strings.Contains(got, "relay-1") {
		t.Errorf("expected 'relay-1' in /setexitnode list, got: %q", got)
	}
	if !strings.Contains(got, "relay-3") {
		t.Errorf("expected 'relay-3' in /setexitnode list, got: %q", got)
	}
...
func TestSetExitNodeReplySuccess(t *testing.T) {
	d := setupTestDB(t)
	_, _ = d.Exec(`INSERT INTO exit_servers(node_id, hostname, enabled) VALUES ('relay-1-1', 'relay-1', 1)`)
	got := HandleCommand(context.Background(), userEnv(d), "/setexitnode relay-1-1")
	if !strings.Contains(got, "set to") {
		t.Errorf("expected 'set to' confirmation, got: %q", got)
	}
	if !strings.Contains(got, "relay-1") {
		t.Errorf("expected 'relay-1' hostname in reply, got: %q", got)
	}
	// DB round-trip.
	var got2 string
	_ = d.QueryRow(`SELECT default_exit_node_id FROM portal_users WHERE id = 2`).Scan(&got2)
	if got2 != "relay-1-1" {
		t.Errorf("default_exit_node_id = %q, want %q", got2, "relay-1-1")
	}
	// Audit row.
	var action, detail string
	_ = d.QueryRow(`SELECT action, detail FROM audit_log WHERE user_id = 2 ORDER BY id DESC LIMIT 1`).Scan(&action, &detail)
	...
	if !strings.Contains(detail, "relay-1") {
		t.Errorf("audit detail = %q, expected to contain 'relay-1'", detail)
	}
}
...
func TestSetExitNodeRejectsDisabled(t *testing.T) {
	...
	_, _ = d.Exec(`INSERT INTO exit_servers(node_id, hostname, enabled) VALUES ('relay-3-1', 'relay-3', 1)`)
	_, _ = d.Exec(`INSERT INTO exit_servers(node_id, hostname, enabled) VALUES ('relay-1-1', 'relay-1', 0)`)
	got := HandleCommand(context.Background(), userEnv(d), "/setexitnode relay-1-1")
...
func TestSetExitNodeClear(t *testing.T) {
	d := setupTestDB(t)
	_, _ = d.Exec(`INSERT INTO exit_servers(node_id, hostname, enabled) VALUES ('relay-1-1', 'relay-1', 1)`)
	_, _ = d.Exec(`UPDATE portal_users SET default_exit_node_id = 'relay-1-1' WHERE id = 2`)
...
func TestDefaultExitNodeReplySet(t *testing.T) {
	...
	got := HandleCommand(context.Background(), userEnv(d), "/defaultexitnode")
	if !strings.Contains(got, "relay-1") {
		t.Errorf("expected 'relay-1' hostname in /defaultexitnode reply, got: %q", got)
	}
	if !strings.Contains(got, "relay-1-1") {
		t.Errorf("expected node_id 'relay-1-1' in reply, got: %q", got)
	}
}
...
//  14. admin-only: alice cannot target admin
...
func setupAddRuleTestDB(t *testing.T) *sql.DB {
	...
	_, _ = d.Exec(`INSERT INTO exit_servers(node_id, hostname, enabled) VALUES ('relay-1-1', 'relay-1', 1)`)
	// alice's defaults.
	_, _ = d.Exec(`UPDATE portal_users SET default_device_node_id = '100', default_exit_node_id = 'relay-1-1' WHERE id = 2`)
	return d
}
...
func TestAddRuleReplyRejectsStaleDefaultExitNode(t *testing.T) {
	...
	_, _ = d.Exec(`UPDATE exit_servers SET enabled = 0 WHERE node_id = 'relay-1-1'`)
	_, hs := fakeHeadscale(t)
	got := HandleCommand(context.Background(), userEnvWithHS(d, hs), "/add_rule 1.2.3.4")
...
func TestAddRuleReplyRejectsPerUserLimit(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		_, _ = d.Exec(`INSERT INTO device_rules (user_id, device_id, exit_node_id, target_type, target_value, action, parent_domain) VALUES (2, 100, 'relay-1', 'ip', '1.1.1.1', 'accept', '')`)
	}
	// Cap alice at 5.
...
func TestAddRuleReplyRejectsTotalLimit(t *testing.T) {
	d := setupAddRuleTestDB(t)
	// Seed 3 rules across all users (admin's existing 12
	// already make this > 3, so we use a tighter cap below).
	// We test with a cap of 12 which the seed (12 admin rules)
	// exactly meets.
	env := userEnvWithHS(d, nil)
...
func TestAddRuleReplySuccessIP(t *testing.T) {
	...
	// exit_node_id stored as hostname 'relay-1' (not node_id).
	var exitNodeID string
	_ = d.QueryRow(`SELECT exit_node_id FROM device_rules WHERE user_id = 2 LIMIT 1`).Scan(&exitNodeID)
	if exitNodeID != "relay-1" {
		t.Errorf("exit_node_id = %q, want 'relay-1'", exitNodeID)
	}
...
func TestAddRuleReplyAdminForOtherUser(t *testing.T) {
	d := setupAddRuleTestDB(t)
	_, hs := fakeHeadscale(t)
	// admin (admin) issues /add_rule alice 1.2.3.4.
	...
	// Rule row under alice (id=2), NOT admin (id=1).
	...
	if adminCnt != 0 {
		t.Errorf("expected 0 rules under admin for this target, got %d", adminCnt)
	}
...
func TestAddRuleReplyRejectsNonAdminForOtherUser(t *testing.T) {
	d := setupAddRuleTestDB(t)
	_, hs := fakeHeadscale(t)
	got := HandleCommand(context.Background(), userEnvWithHS(d, hs), "/add_rule admin 1.2.3.4")
	...
	if n != 12 { // only the seeded 12 admin rules
...
func TestDelRuleReplySingleSuccess(t *testing.T) {
	d := setupTestDB(t)
	// Seed alice's rule.
	res, _ := d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES (2, 'relay-1', 'subnet', '1.2.3.4/32', 'accept')`)
	rid, _ := res.LastInsertId()
	_, hs := fakeHeadscale(t)
...
func TestDelRuleReplyMultiSuccess(t *testing.T) {
	...
	r1, _ := d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES (2, 'relay-1', 'subnet', '1.1.1.1/32', 'accept')`)
	r2, _ := d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES (2, 'relay-1', 'subnet', '2.2.2.2/32', 'accept')`)
	r3, _ := d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES (2, 'relay-1', 'subnet', '3.3.3.3/32', 'accept')`)
...
func TestDelRuleReplyDomainCascade(t *testing.T) {
	...
	res1, _ := d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action, parent_domain) VALUES (2, 'relay-1', 'domain', 'telegram.org', 'accept', 'telegram.org')`)
	rid, _ := res1.LastInsertId()
	_, _ = d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action, parent_domain) VALUES (2, 'relay-1', 'subnet', '1.1.1.1/32', 'accept', 'telegram.org')`)
	_, _ = d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action, parent_domain) VALUES (2, 'relay-1', 'subnet', '2.2.2.2/32', 'accept', 'telegram.org')`)
	// One unrelated rule that must NOT be touched.
	_, _ = d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES (2, 'relay-1', 'subnet', '9.9.9.9/32', 'accept')`)
	_, hs := fakeHeadscale(t)
	got := HandleCommand(context.Background(), userEnvWithHS(d, hs), fmt.Sprintf("/delrule %d", rid))
	// 2026-07-16: v0.15.5 — butler-voice format ("Deleted" capital D).
	if !strings.Contains(got, "Deleted 1 rule") {
...
func TestDelRuleReplyAdminForOtherUser(t *testing.T) {
	...
	rid, _ := res.LastInsertId()
	_, hs := fakeHeadscale(t)
	// admin (admin) deletes alice's rule.
...
func TestDelRuleReplyRejectsNonAdminForOtherUser(t *testing.T) {
	d := setupTestDB(t)
	// Seed admin's rule.
	res, _ := d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES (1, 'relay-1', 'subnet', '1.2.3.4/32', 'accept')`)
	rid, _ := res.LastInsertId()
	_, hs := fakeHeadscale(t)
	// alice (non-admin) tries /delrule admin <id>.
	got := HandleCommand(context.Background(), userEnvWithHS(d, hs), fmt.Sprintf("/delrule admin %d", rid))
	// 2026-07-16: v0.15.5 — butler-voice format (capital "Extra").
	if !strings.Contains(got, "Extra args") {
		t.Errorf("expected 'Extra args' rejection for non-admin, got: %q", got)
	}
	// admin's rule untouched.
	var n int
	_ = d.QueryRow(`SELECT COUNT(*) FROM device_rules WHERE id = ?`, rid).Scan(&n)
	if n != 1 {
		t.Errorf("expected admin's rule untouched, got %d rows", n)
	}
}
...
func TestDelRuleIsAliasOfDeleteRule(t *testing.T) {
	...
	res2, _ := d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES (2, 'relay-1', 'subnet', '5.5.5.5/32', 'accept')`)
...
func TestClearRulesReplySendsAlertOnSetPolicyFailure(t *testing.T) {
	d := setupTestDB(t)
	_, _ = d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES (2, 'relay-1', 'subnet', '1.2.3.4/32', 'accept')`)
	_, hs := fakeHeadscaleSetPolicyFail(t)
	notif := newRecordingNotifier()
	env := userEnvWithHS(d, hs)
	env.Notifier = notif
...
func TestClearRulesReplyRejectsNonAdminForOtherUser(t *testing.T) {
	d := setupTestDB(t)
	got := HandleCommand(context.Background(), userEnv(d), "/clearrules admin")
	// 2026-07-16: v0.15.5 — butler-voice format (capital "Extra").
	if !strings.Contains(got, "Extra args") {
...
func TestClearRulesReplyMintForCallerWithRules(t *testing.T) {
	d := setupTestDB(t)
	// Seed alice's rules.
	_, _ = d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES (2, 'relay-1', 'subnet', '1.1.1.1/32', 'accept')`)
	_, _ = d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES (2, 'relay-1', 'subnet', '2.2.2.2/32', 'accept')`)
...
func TestClearRulesReplyDomainCascade(t *testing.T) {
	...
	_, _ = d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action, parent_domain) VALUES (2, 'relay-1', 'domain', 'telegram.org', 'accept', 'telegram.org')`)
	_, _ = d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action, parent_domain) VALUES (2, 'relay-1', 'subnet', '1.1.1.1/32', 'accept', 'telegram.org')`)
	_, _ = d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action, parent_domain) VALUES (2, 'relay-1', 'subnet', '2.2.2.2/32', 'accept', 'telegram.org')`)
	_, _ = d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES (2, 'relay-1', 'subnet', '9.9.9.9/32', 'accept')`)
	_, hs := fakeHeadscale(t)
...
func TestMyExitNodesReplyListsEnabled(t *testing.T) {
	...
	_, _ = d.Exec(`INSERT INTO devices(node_id, last_seen, online) VALUES ('relay-1-1', 1700000000, 1)`)
	...
	if !strings.Contains(got, "relay-1") {
		t.Errorf("expected relay-1 in menu, got: %q", got)
	}
	...
	// check for the relay-1 row substring to confirm the
	// alignment.
	if !strings.Contains(got, "relay-1") || !strings.Contains(got, "relay-1-1") {
		t.Errorf("expected relay-1 row to carry hostname+node_id, got: %q", got)
	}
	if !strings.Contains(got, "online") {
...
func TestMyExitNodesReplyMarksDefault(t *testing.T) {
	...
	// (the relay-1 row should NOT have a ✓).
	if !strings.Contains(got, "✓") {
...
// by setupTestDB (alice = id=2, admin = id=1, IsAdmin=1).
// ---------------------------------------------------------------
