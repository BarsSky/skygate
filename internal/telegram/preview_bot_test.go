...
func TestPreviewBotReplies(t *testing.T) {
	...
	_, _ = d.Exec(`INSERT INTO audit_log(username, action, detail, created_at) VALUES
		...
		('alice', 'rule_added', 'via bot: ip 1.2.3.4 → relay-1 (action=accept, ids=[12])', 1722000100),
		('admin', 'user_create', 'created bob', 1722000200),
	...
	_, _ = d.Exec(`INSERT INTO device_rules(user_id, exit_node_id, target_type, target_value, action) VALUES
		(2, 'relay-1', 'subnet', '91.108.4.0/22', 'accept'),
		(2, 'relay-1', 'subnet', 'github.com/32', 'accept'),
		(2, 'relay-2', 'subnet', 'telegram.org/32', 'accept')`)
	...
	_, _ = d.Exec(`INSERT INTO exit_servers(node_id, hostname, enabled) VALUES
		('relay-1-1', 'relay-1', 1),
		('relay-2-2', 'relay-2', 1)`)
	_, _ = d.Exec(`UPDATE portal_users SET default_exit_node_id = 'relay-2-2' WHERE id = 2`)
