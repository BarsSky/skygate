...
func makeNode(id string, name string, tags []string, expiry time.Time) headscale.HSNode {
	...
	return headscale.HSNode{
		...
		Name:      name,
		GivenName: name,
		User:      headscale.HSUser{Name: "admin", ID: "1"},
		Online:    true,
...
func TestExpireWatch_PicksOnlyNearExpiry(t *testing.T) {
	...
	nodes := []headscale.HSNode{
		...
		makeNode("10", "workstation-2", nil, now.Add(2*time.Second)),
...
// Expiry is nil" so tag:private nodes like workstation-2 /
// workstation-2-1 / Nothing Phone / Base get renewed.
//
// The 4 sub-cases:
//
//  1. tagged + nil Expiry       → skip  (relay-1, relay-2)
//  2. tagged + real Expiry      → renew (workstation-2-style bug)
...
func TestExpireWatch_SkipsOnlyNilExpiry(t *testing.T) {
	now := time.Now()
	nodes := []headscale.HSNode{
		// (1) tagged + nil — relay-1-style exit-node.
		makeNode("11", "relay-1", []string{"tag:exit-node", "tag:public"}, time.Time{}),
		// (2) tagged + 2s — workstation-2-style: tag:private
		// device that registered without a tag and
		// picked up the 2-4s client bug. Must renew.
		makeNode("10", "workstation-2", []string{"tag:private"}, now.Add(2*time.Second)),
	...
	got := f.Renewed()
	if len(got) != 1 {
		t.Fatalf("renewed = %v, want 1 entry (only node 10, workstation-2-style)", got)
	}
	if got[0] != 10 {
