...
// `dns.base_domain: tsnet.example.com` (the operator's
...
//     as `skygate-subnet-<username>.tsnet.example.com`.
...
//     `<device>.skygate-subnet-<username>.tsnet.example.com`.
...
// domain is hardcoded to `tsnet.example.com` for
...
type MagicDNSNames struct {
	...
	// Format: `skygate-subnet-<username>.tsnet.example.com`
	Sidecar string

	// SidecarShort is the hostname-only form
	// (no `.tsnet.example.com` suffix). Useful when
	...
	// Format: `<device>.skygate-subnet-<username>.tsnet.example.com`
