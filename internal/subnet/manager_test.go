...
//
// 2026-08-10 v0.33.1.41 #2: V054 reserves id=99 for the
// 'infra' system user, so AUTOINCREMENT no longer starts
// at 1 for fresh test DBs. The test now computes the
// expected CIDR from the actual seedPortalUser return
// value (which used to be 1, but is now 100+).
func TestCreateAndGet(t *testing.T) {
	...
	if s.CIDR != wantCIDR {
		t.Errorf("CIDR = %q, want %q (user_id=%d)", s.CIDR, wantCIDR, uid)
	}
