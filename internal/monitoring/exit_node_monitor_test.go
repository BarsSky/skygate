...
func TestTick_TransitionOnlineToOffline_FiresAlert(t *testing.T) {
	...
	if !contains(calls[0], "relay-1") || !contains(calls[0], "online") || !contains(calls[0], "offline") {
...
func TestTick_GarbageCollectsStaleNodes(t *testing.T) {
	...
	// Tick 2: relay-1 removed from headscale.
	hs.setNodes(nil)
	if err := m.tick(context.Background()); err != nil {
...
func TestFormatAlert_IncludesHostnameAndTransition(t *testing.T) {
	...
	got := formatAlert(sc)
	for _, want := range []string{"relay-1", "online", "offline", "went offline"} {
...
// pre-existing relay-1 row stays at tag:untagged, no new row
// for relay-2.
func TestTick_AutoSyncDisabled_DoesNotWriteNodeOwnerMap(t *testing.T) {
	...
	// Pre-seed relay-1 (id=3) with the wrong tag.
	if err := db.UpsertNodeOwner(d, "3", 1, "admin", "tag:untagged", 0); err != nil {
		t.Fatalf("seed relay-1: %v", err)
	}
	...
	// relay-1's row should be UNCHANGED (still tag:untagged).
	row, err := db.GetNodeOwner(d, "3")
	if err != nil {
		t.Fatalf("GetNodeOwner(relay-1) = %v", err)
	}
	if row.Tag != "tag:untagged" {
		t.Errorf("relay-1 tag = %q, want 'tag:untagged' (sync disabled)", row.Tag)
	}

	// relay-2 should NOT have a row.
	if _, err := db.GetNodeOwner(d, "4"); err == nil {
		t.Errorf("GetNodeOwner(relay-2) returned row, want ErrNodeOwnerNotFound (sync disabled)")
	}
