...
func GenerateDockerSteps(current, target, owner, repo string) ManualSteps {
	...
	// 2026-08-11 (v0.34 fix): the previous version left owner/repo
	// in the signature but never used them. The git fetch on the
	...
	return ManualSteps{
		...
		Steps: []string{
			...
			"#    Pin the remote to the canonical owner/repo (v0.34 — was missing).",
			"git remote set-url origin https://github.com/" + owner + "/" + repo + ".git",
			"#    --force allows overwriting local tags that point at orphaned commits.",
			...
			"#    NOTE: 'migrate-only' is a SUBCOMMAND, not the flag",
			"#    '--migrate-only' (that prints 'unknown command')",
