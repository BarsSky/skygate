...
// adminCtx returns an *auth.Claims for the seeded admin
...
func adminCtx(app *App) *auth.Claims {
	return &auth.Claims{UserID: 1, Username: "admin", IsAdmin: true}
}
...
// the session cookie for admin; runs it through
...
func invokeSendTest(t *testing.T, app *App, csrfCookie *http.Cookie, form url.Values) *httptest.ResponseRecorder {
	...
	req.AddCookie(sessionCookieFor(t, app, 1, "admin", true))
