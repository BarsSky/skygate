...
type App struct {
	...
	DerpBaseURL  string // base URL of the local custom DERP server, e.g. http://192.0.2.1:8443
...
func httpGet(url string, timeout time.Duration) ([]byte, error) {
	...
	req.Host = "derp.example.com"
	req.Header.Set("Host", "derp.example.com")
...
const derpPeerNPM = "192.0.2.67"

var (
	derpTailscaleNet = net.IPNet{IP: net.ParseIP("100.64.100.0").To4(), Mask: net.CIDRMask(10, 32)}
	derpLANNet       = net.IPNet{IP: net.ParseIP("192.168.13.0").To4(), Mask: net.CIDRMask(24, 32)}
)

// classifyDerpPeer labels a connection source.
//   ws_relay - Tailscale client (100.64.100.0/10 or any public IP hitting derper)
//   ws_admin - Nginx Proxy Manager WebSocket pool (192.0.2.67)
