...
//   domain hits IPs that relay-3 has NEVER advertised.
//
...
// tables. See newAppForExitRulesTest.
var parentDomainTestCounter int64

// newAppForExitRulesTest builds an App with an in-memory DB
...
func newAppForExitRulesTest(t *testing.T) (*App, *sql.DB) {
	t.Helper()
	...
	if _, err := d.Exec(`INSERT INTO portal_users(id, username, is_admin) VALUES (1, 'admin', 1)`); err != nil {
		t.Fatalf("seed admin: %v", err)
	}
	if _, err := d.Exec(`INSERT INTO portal_users(id, username, is_admin) VALUES (2, 'user1', 0)`); err != nil {
...
func TestInsertRuleUnique_ParentDomainFromDNSResolve(t *testing.T) {
	...
	ok, newID := a.insertRuleUnique(userID, deviceID, exitNode,
		"subnet", ip+"/32", "accept", "100.64.100.9", domain)
	if !ok {
...
func TestInsertRuleUnique_DomainRuleStillSetsParentDomain(t *testing.T) {
	...
	ok, newID := a.insertRuleUnique(userID, deviceID, exitNode,
		"domain", domain, "accept", "100.64.100.9", domain)
	if !ok {
...
func TestFormAddDomain_Flow32RulesHaveParentDomain(t *testing.T) {
	...
	if ok, _ := a.insertRuleUnique(userID, deviceID, exitNode,
		"domain", domain, "accept", "100.64.100.9", domain); !ok {
		t.Fatalf("insert domain rule")
	}
	// 2. /32 rules (with the fix, parentDomain is the domain).
	for _, ip := range resolvedIPs {
		if ok, _ := a.insertRuleUnique(userID, deviceID, exitNode,
			"subnet", ip+"/32", "accept", "100.64.100.9", domain); !ok {
...
func TestDomainAutoUpdater_NoChurnForStableDomain(t *testing.T) {
	...
	added1, removed1 := simulateAutoupdaterTick(t, a, userID, deviceID, exitNode, domain, ips)
	if added1 != len(ips) {
	...
	added2, removed2 := simulateAutoupdaterTick(t, a, userID, deviceID, exitNode, domain, ips)
	if added2 != 0 {
...
func TestDomainAutoUpdater_UpdatesOnIPRotation(t *testing.T) {
	...
	added1, removed1 := simulateAutoupdaterTick(t, a, userID, deviceID, exitNode, domain, ipsTick1)
	...
	added2, removed2 := simulateAutoupdaterTick(t, a, userID, deviceID, exitNode, domain, ipsTick2)
	if added2 != 2 {
...
func simulateAutoupdaterTick(t *testing.T, a *App, userID int64, deviceID int, exitNode, domain string, newIPs []string) (added, removed int) {
	t.Helper()
	d := a.DB
...
func TestDomainAutoUpdater_SharedIPAcrossDomains(t *testing.T) {
	...
	if ok, _ := a.insertRuleUnique(userID, deviceID, exitNode,
		"domain", domainA, "accept", "100.64.100.9", domainA); !ok {
		t.Fatalf("insert domainA")
	}
	added, _ := simulateAutoupdaterTick(t, a, userID, deviceID, exitNode, domainA, []string{sharedIP})
	if added != 1 {
	...
	if ok, _ := a.insertRuleUnique(userID, deviceID, exitNode,
		"domain", domainB, "accept", "100.64.100.9", domainB); !ok {
