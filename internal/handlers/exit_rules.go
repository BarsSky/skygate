...
func (a *App) GenerateACL() (string, error) {
	...
	// base_domain ("tsnet.example.com") for now — it is the only deployment.
	...
	// are routed via the per-device * 100.64.100.0/10 lookup at the SRC IP.
	...
	// devices. tag:public and tag:exit-node remain admin-only (admin)
