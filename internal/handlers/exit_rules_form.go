...
// exit_rules_form.go — extracted from exit_rules.go.
// HTML form handlers for exit-rules:
// - GetMyExitRules       (/my/exit-rules)
// - PostMyExitRule      (/my/exit-rules, add a single rule)
// - PostDeleteExitRule  (/my/exit-rules/delete, single or multi)
// - AdminExitRules      (/admin/exit-rules, list all rules)
// - PostAdminRollbackACL (/admin/exit-rules/rollback)
//
// These handlers render HTML pages and accept form-encoded POSTs.
// They share DB / GenerateACL / saveACLSnapshot / insertRuleUnique
// which remain in exit_rules.go.
