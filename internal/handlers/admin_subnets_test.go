...
func TestGetAdminSubnets_EmptyAndWithRows(t *testing.T) {
	...
	// Empty.
	req := authedReqFor(t, a, "GET", "/admin/subnets", nil, "admin")
...
func TestGetAdminSubnets_StatusFilter(t *testing.T) {
	...
	req := authedReqFor(t, a, "GET", "/admin/subnets?status=active", nil, "admin")
	w := httptest.NewRecorder()
	a.GetAdminSubnets(w, req)
	body := w.Body.String()
	...
	req = authedReqFor(t, a, "GET", "/admin/subnets?status=pending", nil, "admin")
	w = httptest.NewRecorder()
	a.GetAdminSubnets(w, req)
	body = w.Body.String()
	if strings.Contains(body, "alice-filter") {
	...
	req = authedReqFor(t, a, "GET", "/admin/subnets?status=disabled", nil, "admin")
	w = httptest.NewRecorder()
	a.GetAdminSubnets(w, req)
	body = w.Body.String()
