...
// helper (which seeds admin=id=1, alice=id=2 — matching the
...
func TestGetAdminControlPlanes_200ForAdmin(t *testing.T) {
	...
	defer d.Close()
	a.withTemplates()
	req := authedReqFor(t, a, "GET", "/admin/control-planes", nil, "admin")
	...
	if !strings.Contains(body, "admin") || !strings.Contains(body, "alice") {
...
func TestGetAdminUserControlPlane_GET(t *testing.T) {
	...
	defer d.Close()
	a.withTemplates()
	req := authedReqFor(t, a, "GET", "/admin/users/2/plane", nil, "admin")
...
func TestPostAdminUserControlPlane_SaveAndReflect(t *testing.T) {
	...
	req := authedReqFor(t, a, "POST", "/admin/users/2/plane", form, "admin")
	req.SetPathValue("id", "2")
	w := httptest.NewRecorder()
...
func TestPostAdminUserControlPlane_Clear(t *testing.T) {
	...
	// Clear.
	req := authedReqFor(t, a, "POST", "/admin/users/2/plane/clear", nil, "admin")
	req.SetPathValue("id", "2")
	w := httptest.NewRecorder()
...
func TestPostAdminControlPlanesTest_GlobalPlaneOK(t *testing.T) {
	...
	req := authedReqFor(t, a, "POST", "/admin/control-planes/test", form, "admin")
	w := httptest.NewRecorder()
