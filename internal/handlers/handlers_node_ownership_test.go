...
// has tags (e.g. skygate-host-1 has tag:private in headscale,
...
func TestBackfillHelper_StrategyA_TaglessNode_PinsV0222Fix(t *testing.T) {
	...
	_ = db.InsertIgnoreNodeOwnerWithHostname(
		d, "15", 1, "admin", "tag:private", "workstation-3", 1)
	...
	_ = db.InsertIgnoreNodeOwnerWithHostname(
		d, "15", 1, "admin", "tag:untagged", "workstation-3", 1)
	_ = db.UpgradeStaleNodeOwnerToPrivate(d, "15", "tag:private", 1)
	if err := d.QueryRow(
		`SELECT tag FROM node_owner_map WHERE node_id = 15`,
	).Scan(&tag); err != nil {
...
// in headscale (e.g. skygate-host-1, which has forcedTags
...
func TestBackfillHelper_TaglessNode_NoUpgradeFromPrivate(t *testing.T) {
	...
	if _, err := d.Exec(
		`INSERT INTO node_owner_map
		 ...
		 VALUES (13, 1, 'admin', 'tag:private', 1, 'skygate-host-1')`); err != nil {
		t.Fatalf("seed snapshot: %v", err)
	}
	// Re-apply with the helper. Should be a no-op
	// (snapshot already exists, no upgrade needed).
	_ = db.InsertIgnoreNodeOwnerWithHostname(
		d, "13", 1, "admin", "tag:private", "skygate-host-1", 1)
	...
	if tag != "tag:private" {
		t.Errorf("skygate-host-1 tag flipped to %q, want tag:private (regression!)", tag)
	}
}
