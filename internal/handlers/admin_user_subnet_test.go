...
func TestGetAdminUserSubnet_NoSubnet(t *testing.T) {
	...
	req := authedReqFor(t, a, "GET", "/admin/users/2/subnet", nil, "admin")
	_ = req
	// Build the request with a /subnet suffix URL.
	req = authedReqForURL(t, a, "GET", "/admin/users/"+itoa(uid)+"/subnet", "admin")
...
func TestPostAdminUserSubnetAllocateAndDisable(t *testing.T) {
	...
	// Allocate.
	allocReq := authedReqForURL(t, a, "POST", "/admin/users/"+itoa(uid)+"/subnet/allocate", "admin")
	...
	getReq := authedReqForURL(t, a, "GET", "/admin/users/"+itoa(uid)+"/subnet", "admin")
	...
	allocReq2 := authedReqForURL(t, a, "POST", "/admin/users/"+itoa(uid)+"/subnet/allocate", "admin")
	...
	// Disable.
	disReq := authedReqForURL(t, a, "POST", "/admin/users/"+itoa(uid)+"/subnet/disable", "admin")
...
func TestPostAdminUserSubnetTestSanity(t *testing.T) {
	...
	testReq := authedReqForURL(t, a, "POST", "/admin/users/"+itoa(uid)+"/subnet/test", "admin")
...
func authedReqFor(t *testing.T, a *App, method, path string, form url.Values, username string) *http.Request {
	...
	default:
		t.Fatalf("unknown username %q (only admin + alice are pre-seeded)", username)
	}
...
func TestGetAdminUserSubnet_PopulatesSidebarUsername(t *testing.T) {
	...
	body := w.Body.String()
	if !strings.Contains(body, `class="user-name">admin`) {
		t.Errorf("sidebar username empty (c=nil regression) — expected 'admin' inside <span class=\"user-name\">, body excerpt: %q",
...
func TestPostAdminUserSubnetProvision_IssuesPreauthAndShows(t *testing.T) {
	...
	provReq := authedReqForURL(t, a, "POST", "/admin/users/"+itoa(uid)+"/subnet/provision", "admin")
