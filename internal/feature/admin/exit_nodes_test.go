...
func TestComputeSyncStatus_EmptyExpected(t *testing.T) {
	got := computeSyncStatus("emilia", 14, map[string]int{})
	if got != "" {
...
func TestComputeSyncStatus_Synced(t *testing.T) {
	got := computeSyncStatus("karolina", 148, map[string]int{
		"karolina": 148,
	})
	if got != "synced" {
...
func TestComputeSyncStatus_MismatchReversed(t *testing.T) {
	got := computeSyncStatus("emilia", 20, map[string]int{
		"emilia": 14,
	})
...
func TestComputeSyncStatus_OtherNodesIgnored(t *testing.T) {
	expected := map[string]int{
		"emilia":    14,
		"karolina":  357, // mismatch
		"sharlotta": 999, // mismatch
	}
	if got := computeSyncStatus("emilia", 14, expected); got != "synced" {
		t.Errorf("emilia should be synced, got %q", got)
	}
	if got := computeSyncStatus("karolina", 148, expected); got != "mismatch: have 148, want 357" {
		t.Errorf("karolina should mismatch, got %q", got)
	}
	if got := computeSyncStatus("sharlotta", 10, expected); got != "mismatch: have 10, want 999" {
		t.Errorf("sharlotta should mismatch, got %q", got)
	}
...
func TestSeedNodeRulesAndReadExpected(t *testing.T) {
	...
	// 357 device_rules targeting karolina but only 148 routes
	// approved in headscale → mismatch.
	if got := computeSyncStatus("karolina", 148, expected); got != "mismatch: have 148, want 357" {
		t.Errorf("karolina: expected mismatch wording, got %q", got)
	}
	// emilia: 14 == 14 → synced.
	if got := computeSyncStatus("emilia", 14, expected); got != "synced" {
		t.Errorf("emilia: expected synced, got %q", got)
	}
	// sharlotta: 10 == 10 → synced.
	if got := computeSyncStatus("sharlotta", 10, expected); got != "synced" {
		t.Errorf("sharlotta: expected synced, got %q", got)
	}
