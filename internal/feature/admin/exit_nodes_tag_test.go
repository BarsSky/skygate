// 2026-07-30: refactor-v0.30 Phase B step 3b.3 follow-up - ported from
// internal/handlers/admin_exit_nodes_tag_test.go. The 5 tests use
// the shared testutil.go helpers (newTestService, authedReqFor).
// The handlers moved from *App to *Service; the form & redirect
// logic is identical.
//
// 5 tests pin the v0.18.1 contract for the "Tag as exit-node"
// and "Untag" forms on /admin/exit-nodes:
//   - TestPostAdminExitNodeTagAsExitNode_MissingNodeID
//   - TestPostAdminExitNodeTagAsExitNode_BadNodeID
...
// is covered by manual deploy verification on the VM (the
// handler shells out to `docker exec headscale headscale nodes
// ...` which can't be unit-tested in a pure Go test).
...
func TestPostAdminExitNodeTagAsExitNode_MissingNodeID(t *testing.T) {
	...
	req := authedReqFor(t, "POST", "/admin/exit-nodes/tag-as-exit", form, "skyadmin", true)
	w := httptest.NewRecorder()
	s.PostAdminExitNodeTagAsExitNode(w, req)
...
func TestPostAdminExitNodeUntagAsExitNode_BadNodeID(t *testing.T) {
	...
	req := authedReqFor(t, "POST", "/admin/exit-nodes/untag", form, "skyadmin", true)
	w := httptest.NewRecorder()
	s.PostAdminExitNodeUntagAsExitNode(w, req)
...
func TestPostAdminExitNodeTagAsExitNode_Forbidden(t *testing.T) {
	...
	req := authedReqFor(t, "POST", "/admin/exit-nodes/tag-as-exit", form, "alice", false)
	w := httptest.NewRecorder()
	s.PostAdminExitNodeTagAsExitNode(w, req)
...
func TestPostAdminExitNodeUntagAsExitNode_Forbidden(t *testing.T) {
	...
	form := url.Values{}
	form.Set("node_id", "1")
	req := authedReqFor(t, "POST", "/admin/exit-nodes/untag", form, "alice", false)
	w := httptest.NewRecorder()
	s.PostAdminExitNodeUntagAsExitNode(w, req)
