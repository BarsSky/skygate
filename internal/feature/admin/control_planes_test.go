...
func adminPlaneApp(t *testing.T) *Service {
	t.Helper()
	s := newTestService(t)
	// Seed admin (id=1) + alice (id=2) — same as the
	// original newTestApp helper.
	seedPortalUser(t, s.DB, "admin", true)
...
func TestPostAdminUserControlPlane_SaveAndReflect(t *testing.T) {
	...
	req := authedReqFor(t, "POST", "/admin/users/2/plane", form, "admin", true)
...
func TestPostAdminUserControlPlane_Clear(t *testing.T) {
	...
	// Clear.
	req := authedReqFor(t, "POST", "/admin/users/2/plane/clear", nil, "admin", true)
	req.SetPathValue("id", "2")
	w := httptest.NewRecorder()
...
func TestPostAdminControlPlanesTest_GlobalPlaneOK(t *testing.T) {
	...
	req := authedReqFor(t, "POST", "/admin/control-planes/test", form, "admin", true)
	w := httptest.NewRecorder()
