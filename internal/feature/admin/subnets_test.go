...
func TestGetAdminSubnets_EmptyAndWithRows(t *testing.T) {
	...
	// Empty.
	req := authedReqForURL(t, "GET", "/admin/subnets", "skyadmin", true)
...
func TestGetAdminSubnets_StatusFilter(t *testing.T) {
	...
	req := authedReqForURL(t, "GET", "/admin/subnets?status=active", "skyadmin", true)
	w := httptest.NewRecorder()
	s.GetAdminSubnets(w, req)
	body := w.Body.String()
	...
	req = authedReqForURL(t, "GET", "/admin/subnets?status=pending", "skyadmin", true)
	w = httptest.NewRecorder()
	s.GetAdminSubnets(w, req)
	body = w.Body.String()
	if strings.Contains(body, "alice-filter") {
	...
	req = authedReqForURL(t, "GET", "/admin/subnets?status=disabled", "skyadmin", true)
	w = httptest.NewRecorder()
	s.GetAdminSubnets(w, req)
	body = w.Body.String()
