// 2026-07-30: refactor-v0.30 Phase B step 3b.1a follow-up - ported from
// internal/handlers/handlers_telegram_probe_test.go. The tests use
// only pure functions (probeTelegramAPIWithBase, classifyRoute,
// isRouteViaTailscale, defaultRouteViaTailscale, routeViaTailscaleFn
// package var) and httptest for the HTTP mock. No DB / *App / *Service
// setup needed for 14 of 17; the last 3 (cache state) use the
// helper newTestService below (a *Service with a non-nil DB).
//
// 17 unit tests cover the Этап 14 v2 reachability probe:
//   - 3 OK states (direct, relay, direct+tailscaled)
//   - 1 partial relay (mixed state)
//   - 4 classifyRoute branches
//   - 4 unreachable (5xx, conn refused, empty token, 4xx-is-OK)
//   - 1 context cancellation
//   - 1 state string
//   - 2 isRouteViaTailscale (hanging, timeout)
//   - 1 classifyRoute concurrency safety
//   - 3 cached probe (TTL, re-probe on token change, invalidate)
//
// The fields/methods moved from *App to *Service.telegramProbeCache
// (the cache is now a sub-struct on Service rather than 3 separate
// fields on App):
//   app.telegramProbeCache.result       -> s.telegramProbeCache.result
//   app.telegramProbeCache.at           -> s.telegramProbeCache.at
//   app.telegramProbeCache.tokenFP      -> s.telegramProbeCache.tokenFP
//   app.telegramProbeCache.mu           -> s.telegramProbeCache.mu
//   app.cachedTelegramProbe()     -> s.cachedTelegramProbe()
//   app.invalidateTelegramProbe() -> s.invalidateTelegramProbe()
...
func TestCachedTelegramProbeReturnsCachedWithinTTL(t *testing.T) {
	...
	app.telegramProbeCache.result = cached
	app.telegramProbeCache.at = time.Now() // fresh
	app.telegramProbeCache.tokenFP = "fp-same"
...
func TestCachedTelegramProbeReProbesOnTokenChange(t *testing.T) {
	app := newTestService(t)
	app.telegramProbeCache.result = TelegramProbeResult{
		State:   ProbeOKDirect,
		Message: "stale: from a different token",
	}
	app.telegramProbeCache.at = time.Now()
	app.telegramProbeCache.tokenFP = "fp-old"
	...
	app.telegramProbeCache.mu.Lock()
	stillOld := app.telegramProbeCache.tokenFP
	at := app.telegramProbeCache.at
	app.telegramProbeCache.mu.Unlock()
	// 2026-07-30 (v0.34 fix): the original test had an empty
	// `if` body here — the "OK: cache was refreshed" comment
	// was the documentation, but the assertion was missing.
	// The correct behaviour is: a token change MUST clear the
	// cache (tokenFP updates to "fp-new" OR `at` falls below
	// the recent threshold). If the cache still has the old
	// FP AND a recent timestamp, the cache wasn't cleared.
	if stillOld == "fp-old" && at.After(time.Now().Add(-time.Second)) {
		t.Errorf("cache not cleared after token change: stillOld=%q at=%v", stillOld, at)
	}
...
func TestInvalidateTelegramProbeClearsCache(t *testing.T) {
	app := newTestService(t)
	app.telegramProbeCache.result = TelegramProbeResult{State: ProbeOKRelay, Message: "old"}
	app.telegramProbeCache.at = time.Now()
	app.telegramProbeCache.tokenFP = "fp-x"

	app.invalidateTelegramProbe()

	app.telegramProbeCache.mu.Lock()
	at := app.telegramProbeCache.at
	res := app.telegramProbeCache.result
	fp := app.telegramProbeCache.tokenFP
	app.telegramProbeCache.mu.Unlock()
	if !at.IsZero() {
