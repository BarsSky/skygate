...
// regression test. A node with tag:dev-michail-base (per-user
// device) must refuse tag:exit-node.
func TestNodeTagRefused_ExitNodeOnUserDevice(t *testing.T) {
	current := []string{"tag:dev-michail-base", "tag:private"}
	...
	if !strings.Contains(msg, "tag:dev-michail-base") {
	...
	if hadTag != "tag:dev-michail-base" {
		t.Errorf("existingDevTag should be the dev tag, got %q", hadTag)
	}
}

// TestNodeTagRefused_PerRelayExitTag — also refused. tag:exit-emilia
...
func TestNodeTagRefused_PerRelayExitTag(t *testing.T) {
	current := []string{"tag:dev-skyadmin-skyworker", "tag:private"}
	refused, msg, _ := nodeTagRefusedForUserDevice(9, "tag:exit-emilia", current)
	if !refused {
		t.Fatalf("expected refuse for tag:exit-emilia on per-user device, got refused=false msg=%q", msg)
	}
...
func TestNodeTagAllowed_ExitNodeOnRelay(t *testing.T) {
	current := []string{"tag:public"} // emilia-style: no dev tag
...
func TestNodeTagAllowed_PrivateOnUserDevice(t *testing.T) {
	current := []string{"tag:dev-michail-base"}
...
func TestNodeTagAllowed_SubnetRouterOnUserDevice(t *testing.T) {
	current := []string{"tag:dev-skyadmin-skyworker"}
...
func TestNodeTagRefused_ExitNodeOnMultipleDevTags(t *testing.T) {
	current := []string{
		"tag:dev-skyadmin-skyworker",
		"tag:dev-skyadmin-skybars",
		"tag:private",
	}
	refused, msg, hadTag := nodeTagRefusedForUserDevice(9, "tag:exit-karolina", current)
	if !refused {
