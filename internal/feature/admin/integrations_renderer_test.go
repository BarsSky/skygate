...
func TestPostAdminDerpConfig_ActionApply(t *testing.T) {
	...
	req := authedReqFor(t, "POST", "/admin/derp/config", form, "skyadmin", true)
...
func TestPostAdminDerpConfig_ActionTest(t *testing.T) {
	...
	body := w.Body.String()
	// The data-dump testBackend writes "TestResults=[...]"
	// into the body; the original test checked for a
	// template-rendered "Test results" heading which the
	// no-op backend can't produce.
	if !strings.Contains(body, "TestResults=") {
		t.Errorf("expected TestResults in data dump, got body: %s", body)
	}
...
func TestPostAdminHeadplane_ActionApply(t *testing.T) {
	...
	form.Set("action", "apply")
	req := authedReqFor(t, "POST", "/admin/headplane", form, "skyadmin", true)
	w := httptest.NewRecorder()
	...
	// DB has the new mode.
	var mode string
	_ = s.DB.QueryRow(`SELECT value FROM global_settings WHERE key = 'headplane.mode'`).Scan(&mode)
	if mode != "off" {
...
//
// The renderer hardcodes /app/deploy/templates (the
// production in-container path). On Windows we can't
// create /app (it would map to C:\app which requires
// elevation) and even if we did the renderer's path
// is hardcoded. So we skip on Windows — the test is
// most useful on Linux / inside the container.
func setupProductionTemplatesDir(t *testing.T) {
	t.Helper()
	if runtime.GOOS == "windows" {
		t.Skip("renderer needs /app/deploy/templates (hardcoded Unix path); skipping on Windows")
		return
	}
