// 2026-07-30: refactor-v0.30 Phase B step 3b.6 follow-up - ported from
// internal/handlers/admin_backup_config_test.go. The 10 tests use
// the shared testutil.go helpers (newTestService, authedReqFor).
// The handlers moved from *App to *Service; the form & redirect
// logic is identical.
//
// The original used sessionCookieFor (real JWT mint) — we use
// the simpler authedReqFor + X-Test-User header via the
// testBackend.CurrentUser shim. The production JWT path is
// covered by the integration smoke test (scripts/smoke.sh).
//
// 10 tests pin the v0.15.6 contract for the /admin/backup/config
// UI: requiresAdmin, rendersForAdmin, persistsFields,
// autoDetectProtocol, rejectsEmptyDestination, parsesSMB,
// reportsMissingFields, toggleFlipsFlag,
// refusesEmptyDestination (run), requiresAdmin (run).
...
//
// We use the in-memory DB + newTestService helper from
// testutil.go (already imports the same schema). The schema
// is sufficient — global_settings, portal_users, audit_log —
...
// Uses authedReqFor (X-Test-User header) instead of JWT cookies
// so the test is independent of the JWT secret.
func hitConfig(t *testing.T, s *Service, method, path string, body url.Values) *httptest.ResponseRecorder {
	t.Helper()
	var req *http.Request
	if body != nil {
		req = authedReqFor(t, method, path, body, "skyadmin", true)
	} else {
		req = authedReqFor(t, method, path, nil, "skyadmin", true)
	}
	w := httptest.NewRecorder()
	switch {
	case method == "GET" && path == "/admin/backup/config":
		s.GetAdminBackupConfig(w, req)
	case method == "POST" && path == "/admin/backup/config":
		s.PostAdminBackupConfig(w, req)
	case method == "POST" && path == "/admin/backup/test":
		s.PostAdminBackupTest(w, req)
	case method == "POST" && path == "/admin/backup/run":
		s.PostAdminBackupRun(w, req)
	case method == "POST" && path == "/admin/backup/toggle":
		s.PostAdminBackupToggle(w, req)
	default:
...
func TestGetAdminBackupConfig_RequiresAdmin(t *testing.T) {
	s := newTestService(t)
	req := authedReqFor(t, "GET", "/admin/backup/config", nil, "alice", false)
	w := httptest.NewRecorder()
	s.GetAdminBackupConfig(w, req)
...
func TestGetAdminBackupConfig_RendersForAdmin(t *testing.T) {
	s := newTestService(t)
	w := hitConfig(t, s, "GET", "/admin/backup/config", nil)
...
func TestPostAdminBackupConfig_PersistsFields(t *testing.T) {
	...
	w := hitConfig(t, s, "POST", "/admin/backup/config", form)
...
func TestPostAdminBackupTest_ParsesSMB(t *testing.T) {
	...
	w := hitConfig(t, s, "POST", "/admin/backup/test", form)
	if w.Code != http.StatusSeeOther {
		t.Errorf("expected 303, got %d", w.Code)
	}
...
func TestPostAdminBackupToggle_FlipsFlag(t *testing.T) {
	...
	// Toggle off.
	w := hitConfig(t, s, "POST", "/admin/backup/toggle", url.Values{"enabled": {"0"}})
	if w.Code != http.StatusSeeOther {
		t.Errorf("expected 303, got %d", w.Code)
	}
	cfg, _ := backup.Load(d)
	...
	// Toggle on.
	w = hitConfig(t, s, "POST", "/admin/backup/toggle", url.Values{"enabled": {"1"}})
	cfg, _ = backup.Load(d)
	if !cfg.InAppEnabled {
...
func TestPostAdminBackupRun_RefusesEmptyDestination(t *testing.T) {
	s := newTestService(t)
	w := hitConfig(t, s, "POST", "/admin/backup/run", nil)
	if w.Code != http.StatusSeeOther {
		t.Errorf("expected 303, got %d", w.Code)
	}
	if loc := w.Header().Get("Location"); !strings.Contains(loc, "err=") {
...
func TestPostAdminBackupRun_RequiresAdmin(t *testing.T) {
	s := newTestService(t)
	req := authedReqFor(t, "POST", "/admin/backup/run", nil, "alice", false)
	w := httptest.NewRecorder()
	s.PostAdminBackupRun(w, req)
	if w.Code != http.StatusForbidden {
		t.Errorf("expected 403, got %d", w.Code)
