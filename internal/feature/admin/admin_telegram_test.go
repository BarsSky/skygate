...
// (skyadmin), and runs s.AdminTelegramPost.
func invokeSendTest(t *testing.T, s *Service, csrfCookie *http.Cookie, form url.Values) *httptest.ResponseRecorder {
	...
	// X-Test-UserID=1 (skyadmin) so the testBackend's CurrentUser
	// returns Claims{UserID: 1, Username: "skyadmin", IsAdmin: true}.
	// IsAdmin=true is required for AdminTelegramPost to pass the
	// admin gate.
	req.Header.Set("X-Test-User", "skyadmin")
