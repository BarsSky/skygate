...
func newMemoryDB(t *testing.T) *sql.DB {
	...
	stmts := []string{
		...
		// + v0.33.1.33 B85 (added ssh_port). The production
		// code reads ssh_port via COALESCE in qSelectAllExitServers,
		// so a test schema without the column would Scan-fail
		// silently and the egress selector's Egress.Available
		// would be empty (the symptom of v0.33.1.33's first
		// test run). Adding the column here keeps the test
		// schema in sync with production.
...
// InfraAuditIdentity — v0.33.1.41 — same logic as the *App
// wrapper. If the test DB has a portal_users row with
// username='infra' (V054 creates one) AND the row has a
// non-NULL, non-zero headscale_user_id, return its
// (id, 'infra'); otherwise fall back to (fallbackUID,
// fallbackUsername) so tests that don't seed the infra
// row (or seed it without a headscale link) still work.
func (b *testBackend) InfraAuditIdentity(fallbackUID int64, fallbackUsername string) (int64, string) {
	if b.db == nil {
	...
	).Scan(&id, &hsID); err != nil || !id.Valid || !hsID.Valid || hsID.Int64 == 0 {
		return fallbackUID, fallbackUsername
	}
	return id.Int64, "infra"
}
