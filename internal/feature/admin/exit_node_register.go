...
// exitNodeRegisterView is what AdminExitNodes renders after a
...
// the headscale base domain.
func (s *Service) controlURL() string {
	if v := strings.TrimSpace(envOrEmpty("SKYGATE_TS_LOGIN_SERVER")); v != "" {
		return v
	}
	if v := strings.TrimSpace(envOrEmpty("HEADSCALE_SERVER_URL")); v != "" {
		return v
	}
	return ""
}
...
// exitNodeRegisterContext is a tiny seam for tests that need the
// headscale client without a live request.
var _ = context.Background
var _ = headscale.NodeView{}
