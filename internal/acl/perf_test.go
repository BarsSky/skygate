...
func TestGenerateACL_NoDuplicateHosts(t *testing.T) {
	d := openTestDB(t)
	uid := seedPortalUser(t, d, "alice")
	seedUserExitNodePrefWithVia(t, d, uid, "tag:exit-relay-1", true)
...
func TestGenerateACL_FirstMatchOrdering(t *testing.T) {
	...
	seedUserExitNodePrefWithVia(t, d, alice, "tag:exit-relay-1", true)
	seedUserExitNodePrefWithVia(t, d, bob, "tag:exit-relay-2", true)
	aclStr, err := GenerateACLWithVia(d)
	if err != nil {
		t.Fatalf("GenerateACLWithVia: %v", err)
	}
	...
	for i, g := range grants {
		...
		for _, s := range asList(gm, "src") {
			if strings.HasSuffix(s, "@tsnet.example.com") {
				isPerUser = true
			}
			if s == "*" {
...
func TestGenerateACL_ViaOmittedWhenDisabled(t *testing.T) {
	...
	d := openTestDB(t)
	uid := seedPortalUser(t, d, "alice")
	seedUserExitNodePrefWithVia(t, d, uid, "tag:exit-relay-1", false)
	aclStr, err := GenerateACLWithVia(d)
	if err != nil {
		t.Fatalf("GenerateACLWithVia: %v", err)
	}
...
func seedPerfRules(t *testing.T, d *sql.DB, userID int64, n int) {
	...
	for i := 0; i < n; i++ {
		...
		b2 := i % 256
		_, err := d.Exec(
			`INSERT INTO device_rules (user_id, device_id, exit_node_id, target_type, target_value, action, device_ip, parent_domain, user_name, device_hostname, enabled) VALUES (?, ?, ?, 'subnet', ?, 'accept', '100.64.100.1', '', 'alice', 'perf-dev', 1)`,
...
func BenchmarkGenerateACL_ViaEnabled(b *testing.B) {
	...
	seedUserExitNodePrefWithViaBench(b, d, uid, "tag:exit-relay-1", true)
