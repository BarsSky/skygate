...
func TestGenerateACLWithVia_PerUserViaSkippedWhenOptOut(t *testing.T) {
	...
	seedUserExitNodePrefWithVia(t, d, aliceID, "tag:exit-relay-1", false)
	aclStr, err := GenerateACLWithVia(d)
	if err != nil {
		t.Fatalf("GenerateACLWithVia: %v", err)
	}
	...
	perUserNeedle := `"src": ["alice@tsnet.example.com"], "dst": [` +
		`"alice@tsnet.example.com:*"` +
		`, "autogroup:internet"` +
		`]`
	...
	perUserWithVia := `"src": ["alice@tsnet.example.com"]` +
		`, "dst": [` +
		`"alice@tsnet.example.com:*", "autogroup:internet"` +
		`], "ip": ["*"], "via": ["tag:exit-relay-1"] }`
...
func TestGenerateACLWithVia_PerDeviceGrantSkippedWhenOptOut(t *testing.T) {
	d := openTestDB(t)
	aliceID := seedPortalUser(t, d, "alice")
	seedUserExitNodePrefWithVia(t, d, aliceID, "tag:exit-relay-1", true)
	// workstation-3 has a per-device pref but via_enabled=0.
	seedDeviceExitNodePrefWithVia(t, d, aliceID, "workstation-3", "tag:exit-relay-3", false)
	aclStr, err := GenerateACLWithVia(d)
	if err != nil {
		t.Fatalf("GenerateACLWithVia: %v", err)
	}
	// The per-device grant for workstation-3 MUST NOT appear
	// (via_enabled=0 means the per-device grant is
	// skipped entirely).
	perDeviceGrant := `{ "src": ["tag:dev-alice-workstation-3"], "dst": ["autogroup:internet"], "ip": ["*"], "via": ["tag:exit-relay-3"] }`
	if strings.Contains(aclStr, perDeviceGrant) {
		t.Errorf("v0.28.5: per-device grant must NOT be emitted when via_enabled=0.\nACL:\n%s", aclStr)
	}
	// Any per-device grant for tag:dev-alice-workstation-3 (even
	// without via) must also NOT appear — the opt-out
	// means the device falls back to the per-user
	// grant's coverage.
	anyPerDeviceForMSI := `"src": ["tag:dev-alice-workstation-3"]`
	if strings.Contains(aclStr, anyPerDeviceForMSI) {
		t.Errorf("v0.28.5: no per-device grant for workstation-3 when via_enabled=0.\nACL:\n%s", aclStr)
	}
	// The per-user grant for alice must still be
	// present (with via=tag:exit-relay-1 — her per-user
	...
	perUserWithVia := `"src": ["alice@tsnet.example.com"]` +
		...
		`], "ip": ["*"], "via": ["tag:exit-relay-1"] }`
	if !strings.Contains(aclStr, perUserWithVia) {
		t.Errorf("per-user grant for alice (via_enabled=1) must still have via=relay-1.\nACL:\n%s", aclStr)
	}
}
...
// (admin → relay-1, user1 → relay-2, user2
// → relay-3 all have via_enabled=1 after the
...
func TestGenerateACLWithVia_BackwardsCompat_PerUserViaEnabled(t *testing.T) {
	...
	// Per-user grant for alice must have via=tag:exit-relay-1
	// (the existing pinning is preserved).
	want := `"src": ["alice@tsnet.example.com"]` +
		`, "dst": [` +
		`"alice@tsnet.example.com:*", "autogroup:internet"` +
		`], "ip": ["*"], "via": ["tag:exit-relay-1"] }`
	if !strings.Contains(aclStr, want) {
