...
func TestGenerateACL_PerUserSubnetCIDR(t *testing.T) {
	...
	wantAlice := fmt.Sprintf(
		`"src": ["alice@tsnet.example.com"], "dst": ["alice@tsnet.example.com:*", "%s:*"]`,
		aliceCIDR)
	...
	// "bob@tsnet.example.com:*" should appear but NOT
	// as a multi-CIDR dst (his dst has exactly one entry).
	wantBob := `"src": ["bob@tsnet.example.com"], "dst": ["bob@tsnet.example.com:*"]`
...
func TestGenerateACL_SharedSubnetsExtendDst(t *testing.T) {
	...
	wantBob := fmt.Sprintf(
		`"src": ["bob@tsnet.example.com"], "dst": ["bob@tsnet.example.com:*", "%s:*", "%s:*"]`,
		bobCIDR, aliceCIDR)
	if !strings.Contains(aclStr, wantBob) {
...
func TestGenerateACLIncludesDeviceRules(t *testing.T) {
	...
	_, _ = d.Exec(`INSERT INTO device_rules (user_id, device_id, exit_node_id, target_type, target_value, action, device_ip) VALUES ($1, 42, 'relay-1', 'ip', '1.2.3.4', 'accept', '100.64.100.5')`, uid)
