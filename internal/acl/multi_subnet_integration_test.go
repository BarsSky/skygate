...
//
//   (1) "Переход на новую подсеть для admin не сломает работу
//       маршрутов?" — admin already has 10.0.1.0/24 (the
//       v0.16.0 auto-allocate on operator's admin row); we
...
func ruleFor(t *testing.T, aclStr, username string) map[string]any {
	...
	for _, r := range doc.Acls {
		...
		for _, s := range srcs {
			if s == username+"@tsnet.example.com" {
				return r
			}
		}
	}
...
func TestACLBuilder_MultiUserSubnets_PinIsolation(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		if !containsCIDR(dst, tc.uname+"@tsnet.example.com") {
...
// подсеть для admin не сломает работу маршрутов?"
//
// The live state on the VM as of v0.21.1: admin has
// 10.0.1.0/24 (uid=1). The v0.16.0 auto-allocate
// happened when the operator's admin row was first
// loaded. We verify:
//
//   - admin's per-user rule includes 10.0.1.0/24
//   - admin's rule is otherwise unchanged (no
//     extra CIDRs from other users' allocations)
//   - other users' rules are unchanged (no admin
//     CIDR leak)
//
// The "no-op" property: adding admin's CIDR to the
// per-user rule is the ONLY delta; the rest of the
// policy is structurally identical to the no-admin-
...
func TestACLBuilder_SkyadminMigrationIsolated(t *testing.T) {
	...
	// admin's rule should have own identity + own CIDR.
	adminRule := ruleFor(t, aclStr, "admin")
	if adminRule == nil {
	...
	// NO leak from admin or other users.
	user1Rule := ruleFor(t, aclStr, "user1")
	if user1Rule == nil {
		t.Fatal("user1's rule not found")
	}
	user1Dst := dstList(t, user1Rule)
	if containsCIDR(user1Dst, "10.0.1.0/24") {
		t.Errorf("user1's dst leaks admin's CIDR 10.0.1.0/24; dst=%v", user1Dst)
	}
	if !containsCIDR(user1Dst, "10.0.8.0/24") {
	...
	for _, tc := range []struct {
		...
		if containsCIDR(dst, "10.0.1.0/24") {
			t.Errorf("%s's dst leaks admin's CIDR 10.0.1.0/24; dst=%v", tc.uname, dst)
		}
...
func TestACLBuilder_MultipleSharesToOneGrantee(t *testing.T) {
	...
	bobCIDR := fmt.Sprintf("10.0.%d.0/24", bobID)
	carolCIDR := fmt.Sprintf("10.0.%d.0/24", carolID)
	user2CIDR := fmt.Sprintf("10.0.%d.0/24", user2ID)
	for _, p := range []struct {
		uid  int64
		cidr string
	}{
		{aliceID, aliceCIDR}, {bobID, bobCIDR},
		{carolID, carolCIDR}, {user2ID, user2CIDR},
	} {
		if _, err := d.Exec(`INSERT INTO user_subnets
			(user_id, cidr, status, control_plane_url)
			VALUES ($1, $2, 'active', '')`, p.uid, p.cidr); err != nil {
	...
	// alice, carol, user2 all share their subnets with bob.
	// (bob's own share is a no-op via ErrSelfShare, but the
	// ACL builder doesn't see it — only rows in the shares
	// table are read.)
	for _, grantorID := range []int64{aliceID, carolID, user2ID} {
...
func TestACLBuilder_TagOwnersContainAllPortalUsers(t *testing.T) {
	...
	for _, want := range []string{
		...
		"bob@tsnet.example.com",
		"carol@tsnet.example.com",
	} {
		found := false
...
// the tailnet (the 100.64.100.0/10 range) would be
// re-opened.
...
func TestACLBuilder_MultipleMeshesForOneUser(t *testing.T) {
	...
	bobCIDR := seedUserSubnet(t, d, bobID)
	carolCIDR := seedUserSubnet(t, d, carolID)
	user2CIDR := seedUserSubnet(t, d, user2ID)
	// Mesh A: alice + bob
	createMeshInTest(t, d, "", []int64{aliceID, bobID})
	// Mesh B: alice + carol + user2
	createMeshInTest(t, d, "", []int64{aliceID, carolID, user2ID})
	...
	// user2 (B). 3 mesh-mate CIDRs.
	for _, wantCIDR := range []string{bobCIDR, carolCIDR, user2CIDR} {
