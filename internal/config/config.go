...
type Config struct {
	...
	// standard layout "/home/skyadmin/skygate".
	//
	...
	// unreachable — `chdir /home/skyadmin/skygate` fails
...
func Load() (*Config, error) {
	c := &Config{
		...
		// Otherwise default to /home/skyadmin/skygate.
...
// host's /home/skyadmin/skygate is NOT visible from inside
// the container (only the bind-mounts are), so the orchestrator
// MUST use the in-container path. chdir /home/skyadmin/skygate
// would fail with "no such file or directory" because the
// process has no view of the host's filesystem root.
//
// On a bare/systemd host: "/home/skyadmin/skygate" — the
