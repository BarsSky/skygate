...
func TestProvisionUser_ParsesValidJSON(t *testing.T) {
	...
	bootstrap := writeScript(t, dir, "headscale-bootstrap.sh",
		`{"username":"admin","container":"headscale-admin","url":"http://headscale-admin:50450","api_key":"hskey-api-abc123","http_port":50450,"grpc_port":51450,"metrics_port":52450,"base_domain":"admin.tsnet.example.com","headscale_user_id":42}`,
		"", "0")
	defer withScriptPaths(t, bootstrap, "/nonexistent")()

	got, err := ProvisionUser("admin", 1)
	if err != nil {
	...
	if got.Container != "headscale-admin" {
		t.Errorf("Container = %q, want headscale-admin", got.Container)
	}
	if got.URL != "http://headscale-admin:50450" {
		t.Errorf("URL = %q, want http://headscale-admin:50450", got.URL)
	}
...
func TestProvisionUser_StripsPreJSONOutput(t *testing.T) {
	...
	dir := t.TempDir()
	bootstrap := writeScript(t, dir, "headscale-bootstrap.sh",
		`Pulling headscale-admin ... done
Container headscale-admin  Started
{"username":"admin","container":"headscale-admin","url":"http://headscale-admin:50451","api_key":"hskey-api-xyz789","http_port":50451,"grpc_port":51451,"metrics_port":52451,"base_domain":"admin.tsnet.example.com","headscale_user_id":7}`,
		"", "0")
	defer withScriptPaths(t, bootstrap, "/nonexistent")()
...
func TestProvisionUser_ScriptFails(t *testing.T) {
	...
	dir := t.TempDir()
	bootstrap := writeScript(t, dir, "headscale-bootstrap.sh",
		"", "container headscale-admin already exists", "1")
	defer withScriptPaths(t, bootstrap, "/nonexistent")()

	_, err := ProvisionUser("admin", 1)
	if err == nil {
...
func TestProvisionUser_MissingRequiredFields(t *testing.T) {
	...
	dir := t.TempDir()
	bootstrap := writeScript(t, dir, "headscale-bootstrap.sh",
		`{"username":"admin","http_port":50450}`,
		"", "0")
	defer withScriptPaths(t, bootstrap, "/nonexistent")()
...
func TestDecommissionUser_HappyPath(t *testing.T) {
	...
	deprovision := writeScript(t, dir, "headscale-deprovision.sh",
		"OK: deprovisioned admin", "", "0")
	defer withScriptPaths(t, "/nonexistent", deprovision)()

	if err := DecommissionUser("admin"); err != nil {
...
func TestProvisionResult_JSONRoundTrip(t *testing.T) {
	in := ProvisionResult{
		Username:        "admin",
		Container:       "headscale-admin",
		URL:             "http://headscale-admin:50450",
		APIKey:          "hskey-api-roundtrip",
		HTTPPort:        50450,
		GRPCPort:        51450,
		MetricsPort:     52450,
		BaseDomain:      "admin.tsnet.example.com",
		HeadscaleUserID: 42,
	}
