...
// createPreauthViaCLIWithTags shells out to `docker exec <container> headscale
// preauthkeys create --tags tag1,tag2`. Parses the hskey-... token
// out of stdout (the CLI is the only place that returns the plaintext
// key reliably) and best-effort parses the JSON --output block to
// extract the key ID. headscale 0.23+ requires the --tags flag for
// tag-restricted preauth keys; the no-tags variant is not used by
// any caller (always pass at least one tag).
