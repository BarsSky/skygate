...
### 1. `internal/subnet/magicdns.go` (new)
  ...
  - `subnet.BaseDomain = "tsnet.example.com"` —
...
### 4. Tests
  ...
  - 4 new `magicdns_test.go` tests:
    - `TestComputeMagicDNSNames_KnownUsernames` —
      alice / user1_42 / guest all produce
...
`dns.base_domain: tsnet.example.com` in headscale
...
The wildcard form (`<device>.skygate-subnet-<username>.tsnet.example.com`)
...
The base domain is hardcoded to `tsnet.example.com`
