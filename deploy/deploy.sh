...
if [ "${MODE}" = "restore" ] && [ -d "${FROM_PATH}/ssh" ]; then
    cp "${FROM_PATH}/ssh/"* /home/admin/.ssh/ 2>/dev/null || true
    chmod 600 /home/admin/.ssh/skygate_sync 2>/dev/null || true
    chmod 644 /home/admin/.ssh/skygate_sync.pub 2>/dev/null || true
