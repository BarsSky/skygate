...
catch-all for security (the operator's `msi → karolina` bypass
...
skygate-vm after v0.28.3, even with the per-user grant dst
...
(skygate-vm is a good test since it has no per-device pref):

```bash
docker exec -w /tmp skygate tailscale --socket=/var/run/tailscale/tailscaled.sock set --exit-node=emilia
...
- DB: 3 per-user prefs (all `via_enabled=0`), 6 per-device prefs
  (msi and skybars `via_enabled=1` per the operator's last choice,
  others `via_enabled=0`)
- Exit-node on skygate-vm: WORKS (145ms latency to 8.8.8.8 via
  emilia). Per-device grants emitted for all 11 tagged devices
  (a71, desktop-cuo0tfb, emilia, karolina, msi, sharlotta,
  skybars, skybars-1, skygate-subnet-skyadmin, skygate-vm,
  skyworker).
- Operator: re-test msi (Windows) and Android (skybars).
  Expected: both can pick any exit-node in Tailscale client (no
  more `via` enforcement blocking the selection). msi should
  also use karolina (its `via_enabled=1` pref), skybars should
  use emilia (its `via_enabled=1` pref).
