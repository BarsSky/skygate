...
#   2. CIDR + status pill on /my/devices for admin
#   3. /admin/users subnet column shows the new colored pill
#   4. /admin/users/{id}/subnet page shows the v0.22.3 explainer
#   5. admin status is "active" (has devices) in DB
#   6. Pre-v0.20.0 users (user1/guest/user2) keep their "—" / no subnet
#
# Plus a 7th: the API path works for /my/devices (admin authenticates,
...
if echo "$out" | grep -q "10.0.1.0/24"; then
    echo "PASS: /my/devices shows 10.0.1.0/24 (admin's CIDR)"
else
...
if echo "$out" | grep -q "admin" && echo "$out" | grep -q "10.0.1.0/24"; then
    echo "PASS: /admin/users shows admin's CIDR 10.0.1.0/24"
else
    echo "FAIL: admin's CIDR NOT in /admin/users"
    echo "--- relevant fragment: ---"
    echo "$out" | grep -A 3 "admin" | head -10
    exit 1
fi
...
# Find admin's id from /admin/users
uid=$(echo "$out" | grep -oE '/admin/users/[0-9]+/(subnet|delete)' | head -1 | grep -oE '[0-9]+')
if [ -z "$uid" ]; then
    echo "FAIL: couldn't extract admin's id from /admin/users"
    exit 1
fi
echo "  (admin id=$uid)"
...
echo
echo "--- 5. admin status in DB is 'active' (not 'pending')"
db_status=$(docker exec skygate sqlite3 /data/skygate.db \
    "SELECT subnet_status FROM portal_users WHERE username='admin'" 2>&1)
if [ "$db_status" = "active" ]; then
    echo "PASS: admin's subnet_status = 'active' (was 'pending' pre-v0.22.3)"
else
    echo "FAIL: admin's subnet_status = '$db_status', want 'active'"
    exit 1
fi
db_cidr=$(docker exec skygate sqlite3 /data/skygate.db \
    "SELECT subnet_cidr FROM portal_users WHERE username='admin'" 2>&1)
if [ "$db_cidr" = "10.0.1.0/24" ]; then
    echo "PASS: admin's subnet_cidr = '10.0.1.0/24' (correct deterministic allocation)"
else
    echo "FAIL: admin's subnet_cidr = '$db_cidr', want '10.0.1.0/24'"
    exit 1
fi

echo
echo "--- 6. pre-v0.20.0 users (user1/guest/user2) keep their 'no subnet' state"
for u in user1 guest user2; do
