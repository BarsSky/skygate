...
1. **Real end-to-end proof that the per-user subnet-router
   ...
   pipeline, verified live on the operator's VM (admin
...
**Live result (2026-07-22, admin pilot on
`192.0.2.1`):** node id=26 registered with
...
documents the actual output of the 2026-07-22 admin
...
8. (Optional) `bash scripts/check_subnet_router.sh
   admin` — should report `[OK]` for all 5
   checks.
