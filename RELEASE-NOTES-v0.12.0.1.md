...
network" view (each device's 100.64.100.0/10 Tailscale IP
...
https://github.com/skygate-operator/skygate/releases/tag/v0.12.0.1
...
1. **Critical:** run `/admin/exit-rules/reapply` to push
   ...
   reapply, the audit log records `reapply by admin →
   vN`.
