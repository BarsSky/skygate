...
delete button. Clicking it:
  ...
  1. Approves ONLY the exit-node bases
     ...
     `AvailableRoutes` set — relay-3 has 200+
...
```
sudo tailscale up --login-server=https://head.example.com --ssh --advertise-exit-node --accept-routes
```
