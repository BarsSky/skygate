...
# The 6 known admin devices (verified by the operator on
...
# If admin gets new devices in the future, this list can be
...
override=$(docker exec skygate sh -c "sqlite3 /data/skygate.db 'SELECT headscale_url FROM portal_users WHERE username=\"admin\"'")
if [ -z "$override" ]; then
...
SQL
    ...
    post_override=$(docker exec skygate sh -c "sqlite3 /data/skygate.db 'SELECT headscale_url FROM portal_users WHERE username=\"admin\"'")
...
echo
echo "--- 3. UPDATE node_owner_map: set username='admin' for admin's devices"
device_list=""
...
UPDATE node_owner_map
 ...
 WHERE hostname IN ($device_list)
   AND (username != 'admin' OR username = 'tagged-devices' OR username = '');
SQL
...
echo "    (should now show username=admin for all 6)"
...
post_admin_count=$(echo "$post_state" | grep -c "admin" || true)
if [ "$post_admin_count" = "6" ]; then
    echo "PASS: all 6 devices now have username=admin"
else
    echo "FAIL: only $post_admin_count of 6 devices have username=admin"
    exit 1
fi
...
db_status=$(echo "SELECT subnet_status FROM portal_users WHERE username = 'admin';" \
    | docker exec -i skygate sqlite3 /data/skygate.db)
if [ "$db_status" = "active" ]; then
    echo "PASS: admin's subnet_status = 'active' (was 'pending' before fix)"
else
    echo "FAIL: admin's subnet_status = '$db_status', want 'active'"
...
for h in "${SKYADMIN_DEVICES[@]}"; do
    ...
    # could be substrings, but for admin's device set the
...
if [ "$device_count" -ge 4 ]; then
    echo "PASS: /my/devices renders the admin devices (≥4 of 6 — exact count depends on which are 'online' right now)"
else
