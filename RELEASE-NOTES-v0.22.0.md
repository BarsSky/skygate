...
- `TestACLBuilder_SkyadminMigrationIsolated` — the user's
  question (1): admin's 10.0.1.0/24, no leak to others.
...
2. Subnets allocated for user1 (10.0.6.0/24), guest
   (10.0.9.0/24), user2 (10.0.10.0/24)
...
6. Create a mesh: user1 + guest
7. Re-apply ACL — headscale policy NOW has:
   - user1's rule: `10.0.6.0/24:*` AND `10.0.9.0/24:*` (guest)
   - guest's rule: `10.0.9.0/24:*` AND `10.0.6.0/24:*` (user1)
   - user2's rule: `10.0.10.0/24:*` only (NOT a member)
8. /admin/meshes renders the new mesh (HTTP 200)
9. Dissolve the mesh; re-apply — user1 and guest's rules
...
- The per-user rule shape is unchanged. Each user's rule is
  still `src: [user@tsnet.example.com], dst: [user@*:..., own_cidr:*, shared_cidrs:*, mesh_mate_cidrs:*]`. The mesh
...
cd /home/admin/skygate && make test

# 2. Run the v0.22.0 mesh live validation (7 checks)
scp check_v0.22.0_mesh.sh admin@<vm>:/tmp/
ssh admin@<vm> "chmod +x /tmp/check_v0.22.0_mesh.sh && bash /tmp/check_v0.22.0_mesh.sh"

# 3. Inspect the mesh schema
ssh admin@<vm> "docker exec skygate sqlite3 /data/skygate.db '.schema meshes'"
...
- check_exit_nodes: 3/3 (relay-1, relay-2, relay-3)
