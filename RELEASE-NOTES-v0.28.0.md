...
**any device that picks up the same IP inherits the rule** (e.g. workstation-3 and
desktop accidentally saw workstation-1's rules because their IPs overlapped the
...
that tag instead of the IP. Rules for workstation-1 apply only to workstation-1;
workstation-3 and desktop are isolated.
...
`tag:dev-admin-workstation-1`, `tag:dev-user1-base`, etc. Tags are
...
- **Per-user isolation invariant pinned in test**:
  `TestGenerateACL_PerDeviceTagDoesNotCrossUsers` — admin's
  per-device tag is owned by `admin@tsnet`, never by
  `user1@tsnet`. A rule for workstation-1 cannot apply to workstation-3.
...
- **21 → 41 legacy `device_ip` rules** still in the live policy
  for `100.64.100.1` (workstation-1); these will be cleaned up in v0.28.1
  once we have a story for them (the device's `node_owner_map`
  row is empty, so the v0.28.0 backfill can't auto-recover
  them). Not a security issue — they only apply when workstation-1's
  Tailscale IP happens to be `100.64.100.1`.
...
- `headscale nodes list -o json` — 13/13 user devices carry their
  per-device tag (workstation-1, workstation-3, desktop, workstation-2, workstation-2-1,
  skygate-host-1, skygate-subnet-admin, a71, relay-1, relay-2,
  relay-3, base, workstation-5)
...
curl -sS -c /tmp/ck -b /tmp/ck -X POST --data-urlencode 'username=admin' \
...
curl -sS -c /tmp/ck -b /tmp/ck http://localhost:8080/my/devices | \
  grep -c 'tag:dev-admin-'
# expect: >= 9 (workstation-1's 200+ rules collapse to 1 cell; the 9 are the
...
1. AddTag for relay-1/relay-2/relay-3 with per-exit-node tags
