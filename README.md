...
> advertise `0.0.0.0/0` + `::/0` (relay-1, relay-2, relay-3), `go test ./...`
> race-free.
...
Default admin: `admin` + the password you set in `SKYGATE_ADMIN_PASS`.
...
# Another host on the LAN (e.g. 192.0.2.1):
...
- **Nginx Proxy Manager** (easiest): add proxy host `skygate.example.com`
  → `http://192.0.2.1:8080`, request LE cert, force SSL.
- **Caddy** (one-liner):
  ```
  skygate.example.com {
...
- ✅ Per-user rule limits (`SKYGATE_USER_MAX_RULES=admin:2000,alice:500`)
...
[MIT](LICENSE) — Copyright (c) 2026 skygate-operator. Use, modify, redistribute
