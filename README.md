...
(admin) создавать пользователей и выдавать им доступ к self-service
...
8. Настройте NPM proxy host для skygate.example.com:
   - Domain: `skygate.example.com`
   - Forward: `http://192.0.2.1:8080`
   - SSL: request new LE cert
   - Force SSL: ON

9. Откройте `https://skygate.example.com/` и залогиньтесь как `admin`.
