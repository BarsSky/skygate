...
admin's 6 devices (all `tag:private`, no subnet-router
...
users (admin / user1 / guest / user2) are at the
following state as of v0.24.0 + v0.24.1:

| User     | Subnet           | Subnet-router | Used mesh? | Used share? |
|----------|------------------|---------------|------------|-------------|
| admin | 10.0.1.0/24 act. | none          | no         | no          |
| user1  | 10.0.6.0/24 pen. | none          | no         | no          |
| guest    | 10.0.9.0/24 pen. | none          | no         | no          |
| user2   | 10.0.10.0/24 pen.| none          | no         | no          |
...
- For each of user1/guest/user2, the user needs to
...
using it: have one user (say user1) click
`/my/meshes → Create mesh`, share the code with admin
in chat, admin pastes it into `/my/meshes → Join`.
...
Tailscale client GUI (`tailscale up --exit-node=relay-1`
