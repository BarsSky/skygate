...
```bash
# 1. Get the script
curl -fsSL https://raw.githubusercontent.com/skygate-operator/skygate/main/deploy/subnet-router/setup.sh -o /tmp/setup.sh
...
1. **`deploy/subnet-router/setup.sh`** — the
   ...
   ```
   curl -fsSL https://raw.githubusercontent.com/skygate-operator/skygate/main/deploy/subnet-router/setup.sh -o setup.sh
   chmod +x setup.sh
   ```
   Or use the one-line `curl | bash` form (after you've
   read the script and trust it):
   ```
   curl -fsSL https://raw.githubusercontent.com/skygate-operator/skygate/main/deploy/subnet-router/setup.sh | bash -s --
   ```
...
2. **This very document** — `docs/subnet-router.md`. If
   ...
   have it. Otherwise:
   ```
   curl -fsSL https://raw.githubusercontent.com/skygate-operator/skygate/main/docs/subnet-router.md
   ```
...
operator's VM (`admin` pilot). Re-run the e2e
...
scp C:/Projects/skygate/e2e_pilot.sh admin@<VM>:/tmp/
ssh admin@<VM> "bash /tmp/e2e_pilot.sh"
```

The script:
1. Logs in as `admin`, downloads the bundle from
...
3. Pulls `tailscale/tailscale:latest` and starts a
   ...
   `TS_AUTHKEY`, `TS_LOGIN_SERVER=https://head.example.com`,
   `TS_HOSTNAME=skygate-subnet-admin`.
...
7. Confirms the sidecar log line
   `sidecar: approved 10.0.1.0/24 on node skygate-subnet-admin (user=1)`.
...
bash scripts/check_subnet_router.sh admin
# or
...
The admin opens `https://gate.example.com/admin/users/<your-id>/subnet`,
...
SUBNET_ROUTER_HOSTNAME=skygate-subnet-user1 \
...
```
sidecar: approved 10.0.6.0/24 on node skygate-subnet-user1 (user=6)
```
...
for n in json.load(sys.stdin):
    if 'skygate-subnet-user1' in n.get('name', ''):
        print('id=' + str(n['id']))
...
ping skygate-subnet-user1
# (MagicDNS resolves it via the tailnet's base_domain, e.g. tsnet.example.com)
...
1. Check skygate logs:
   ```bash
   docker logs --since 5m skygate | grep -E 'sidecar.*user1|10.0.6.0/24'
   ```
...
### "ping works to skygate-subnet-user1 but not to 10.0.6.1"
...
### "MagicDNS doesn't resolve skygate-subnet-user1"
...
- [`docs/tailscale-relay.md`](tailscale-relay.md) — same
  ...
  relay-3). Different tag (`tag:exit-node` /
