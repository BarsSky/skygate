...
- **2026-09-11**: v1.5.4 — B-mod-sqlite-pg-bidi (SQLite+Postgres
  bidirectional support + conversion tool). Closes the operator
  2026-09-11 'restore explicit SQLite support alongside PG with
  full conversion' + 'admin chooses DB type at startup' +
  'switch between DBs without data loss' requirements (the v1.3.0
  B-mod-db-retry that removed SQLite was reversed). Live-verify
  on svi polygon is PENDING operator confirmation (Task 7).

  - **B-mod-sqlite-pg-bidi T1** (commit `42caa34f`): Dialect
    interface + DetectDSN. 9 unit tests in
    `internal/db/dialect_test.go` (all 9 DSN forms + placeholders +
    unix epoch + insert-ignore + boolean + live `:memory:` SQLite
    open). Adds `modernc.org/sqlite v1.34.5` dependency (pure-Go,
    no CGO). Foundation for the SQLite + PG dual-support refactor.
  - **B-mod-sqlite-pg-bidi T2** (commit `08cafa35`): SQLite migration
    set (49 functions, v0.20-v0.70, generated by
    `scripts/port_migrations_sqlite.py` reverse-porting from PG
    source). 3 tests in `internal/db/migrations_sqlite_test.go`
    (apply-all + PG-token static scan + key-tables-exist). Adds
    dialect-aware `ensureMigrationTrackingTable` +
    `RecordMigrationApplied`. `migrations_audit_b233_test.go`
    updated to exclude the derived `migrations_sqlite.go` from the
    shape-drift audit.
  - **B-mod-sqlite-pg-bidi T3+T4** (commit `fce937a5`): `db.Convert`
    + `skygate db-migrate` subcommand. 4 conversion tests +
    9 CLI arg-parsing tests. Algorithm: readSourceSchema →
    translateSchema → topoSortTables → emit CREATE TABLE →
    copyTableData (with dialect-native placeholders via
    `DialectKind.Placeholders`). CLI: `--from` / `--to` /
    `--schema-only` / `--data-only` / `--dry-run` / `--help`.
  - **B-mod-sqlite-pg-bidi T6** (commit `cd28030c`): `SKYGATE_DB`
    env var (preferred) + `SKYGATE_DB_DSN` legacy backward compat
    (precedence via `resolveDBDSN`). 4 env-var precedence tests.
    `--db-type=sqlite|postgres` flag on `install-debian.sh` with
    interactive TTY prompt (defaults to sqlite). New
    `docker-compose.sqlite.yml` for SQLite-only deployments (no
    PG service required, bind-mounted `./data/skygate.db`). Empty
    SKYGATE_DB now defaults to local SQLite at DBPath (was fatal
    error pre-v1.5.4). `TestRunMigrateOnly_RespectsDSN` updated
    to set `SKYGATE_DB` directly (the new precedence rule).

  Live-verify Task 7 (PG↔SQLite round-trip на svi polygon) —
  PENDING operator confirmation. The unit tests prove correctness
  on `:memory:` SQLite; the production cross-dialect path needs
  the polygon VM access (operator-stated VM topology: 45.152.198.217
