...
```
add_device: одноразовый ключ на час для admin:
...
5. **Address by name.** «Добрый вечер, admin» —> персонализация.
...
- Live verify: `bash scripts/smoke.sh` (118/118) + manual `/add_device` /
  `/status` / `/nodes` от admin, screenshot в Telegram.
