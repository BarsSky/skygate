...
operator's VM (`admin` pilot). For ongoing regression
coverage, the subnet-router flow is exercised by the
Go test suite:
...
(The one-time `e2e_pilot.sh` shell script that was used
for the original 2026-07-22 live verification has been
removed; the Go test suite is the canonical regression
guard. The 9-step live verification flow it ran is
