...
│       (user=1 cyborg  <DEVICE_Y>  youtube.com)                 │
│       (user=6 basic   <DEVICE_Y>  142.250.0.0/15)              │
...
│       E.g. (user=1 cyborg  tag:dev-infra-<DEVICE_Y> via=1).  │
...
| 4 | existing pref with wrong tag | **UPDATE** | `stale-tag` (e.g. operator typed `tag:dev-<USER_A>-basic` instead of the canonical `tag:dev-infra-<DEVICE_Y>`) |
...
| 7 | orphan user_id (<USER_A> — no portal_users row) | **CREATE** (with empty Username) | the B237.7 root cause category — must not crash on missing usernames |
...
"unanimous <DEVICE_Y> → CREATE pref", and **silently logged
...
# Expect: cyborg / basic rows with tag:dev-infra-<DEVICE_Y>, via=1

# 3. Reapply headscale policy
curl -s -c /tmp/c.txt -b /tmp/c.txt \
  --data-urlencode "username=<OPERATOR_USER>" --data-urlencode "password=admin" \
  https://skygate.example.com/login >/dev/null
...
for g in d.get('grants', []):
    if 'via' in g and 'tag:dev-infra-<DEVICE_Y>' in g.get('via', []):
        print('OK:', g)"
```
...
that user_id without a `portal_users` row (e.g. `<USER_A>`
...
- B236 / `docs/internal/runbooks/tailnet-advertised-routes.md` —
