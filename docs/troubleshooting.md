...
## Prefix assignment (кто обслуживает сеть) — B275 / B275.1

**Симптом.** «У одного устройства сайт открывается, у другого — нет», хотя правила есть у обоих. Или rutracker/Google/Cloudflare работали и перестали после перезапуска синхронизации маршрутов.

**Почему так.** headscale отдаёт каждую подсеть ровно с **одного** exit-узла (primary). Если два устройства разных пользователей просят одну и ту же сеть через разные узлы (`basic` → emilia и `skyworker` → karolina на одних и тех же 28 диапазонах Cloudflare/Google — типовой случай, потому что одно доменное правило разворачивается в весь опубликованный набор CDN), обслужить обоих невозможно: второй анонс клиентам вообще не передаётся. Раньше это выглядело как плавающая поломка, потому что primary выбирался по тому, кто анонсировал последним.

**Как устроено теперь (B275).** Таблица `prefix_owner` (`prefix`, `exit_node_id`, `source`, `claims`, `devices`) — единственный источник истины о том, кто анонсирует префикс:

* `source=explicit` — так решили правила устройств (побеждает большинство, тай-брейк по имени узла);
* `source=manual` — закреплено администратором, движок не переназначает, пока узел здоров;
* `source=auto` — выбрано skygate: наименее загруженный доступный узел, sticky (без причины не переезжает).

Упавший владелец теряет префикс (переназначается). Если headscale недоступен, таблица не пустеет — движок опирается на узлы, которые назвали правила. Правила устройств (`device_rules.exit_node_id`) при этом **не переписываются**: skygate решает, кто физически обслуживает сеть, а per-CIDR грант ACL пиннится на **владельца**, а не на узел из правила.

**Где смотреть и править.** `/admin/exit-nodes` → секция «Распределение префиксов»: колонка «Источник» объясняет выбор, колонка «Анонс» показывает, реально ли узел объявляет сеть («нет» = синк маршрутов не прошёл: нажмите «Синхр.» в строке узла и проверьте SSH), выбор узла в строке + «Сохранить» закрепляет префикс (`SetManual`), выбор «авто» возвращает его движку.

SQL для проверки:

    select prefix, exit_node_id, source, claims, devices from prefix_owner order by prefix;

Логи: `prefix-owner: assignment table updated (inserted=N changed=N)`, `prefix-ownership(staggeredSync): <узел> drops N claimed prefix(es) owned by another relay`.

**Что важно помнить.** Префикс не может обслуживаться двумя узлами одновременно — это ограничение headscale, а не skygate. Если конкретная сеть обязана идти через конкретный узел (например, Telegram только через karolina), закрепите её вручную: движок перестанет её переназначать. Windows/Linux получают сети через `accept-routes` и настраивать на них ничего не нужно; Android/iOS выбирают exit-узел сами в приложении.
