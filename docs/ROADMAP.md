...
| **RR-11** | Flaky Go-load contracts (B183 `[I]`, B213, B235, B237.2) | **Closed for the observed cases (2026-09-19).** Three distinct causes were found and fixed: (1) `B202.5`'s rotating FAIL was a real race — `TestSSHDumpTransport_Dump_FakeSsh` asserted stderr lines the reader goroutine could still be holding when `cmd.Wait()` closed the pipes (both dump transports now drain to EOF before `Wait()`); (2) `check_b237_24`'s live `git ls-remote` probe reported an unreachable remote as `FAIL` instead of `SKIP`; (3) the `skygate <verb> --help` contracts `B211`–`B214` link the whole binary, and on this 2 vCPU / 1.8 GB host the linker occasionally dies under full-gate pressure (each check passes 20/20 standalone) — they now retry the link once and print the real error (`scripts/lib/go_build.sh`). The `GOFLAGS=-p=2` cap and the 180 s budgets stay |
