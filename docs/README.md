...
| [runbooks/svyatoslava-bootstrap.md](runbooks/svyatoslava-bootstrap.md) | <polygon-vm-hostname> (HA mirror) bootstrap — provision new VM, join headscale, install skygate, wire up as a P2 HA member. Excludes the node from `exit_servers` per the operator's "technical HA host, not traffic routing" decision | On-call operators adding the second skygate node |
