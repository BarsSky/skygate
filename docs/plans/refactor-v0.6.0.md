...
**Repo:** github.com/skygate-operator/skygate
...
├── ssh.go         // SetAdvertisedRoutes (SSH на relay-3/relay-1/relay-2)
...
**Главное:** НИЧЕГО не менять в логике, только move. `SetAdvertisedRoutes` — особенно осторожно (только что починили relay-3-bug).
...
2. `internal/headscale/model_test.go` — toView(), hasExitNodeTag() (включая баг relay-3 0.0.0.0/0)
...
1. **SetAdvertisedRoutes** (только что починили relay-3) — НЕ трогать в Этапе 7
