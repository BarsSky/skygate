...
```
operator's devices ─── tailscale0 ──► skygate-vm (192.168.13.69)
                                      ├─ skygate (Go) + sidecar
                                      ├─ headscale (control plane)
                                      ├─ caddy (TLS)
                                      └─ Tailscale client (skygate-vm identity)

daily cron → /var/backups/skygate/latest/
            /var/backups/headscale/latest/
```

- One VM (`192.168.13.69`, public `95.165.170.190`)
...
```
              ...
              │  (skygate-vm)   │  replication     │  (svyatoslava)  │
              │                 │ ◄──────────────► │                 │
...
provides: 2nd VM (svyatoslava), S3 bucket, etcd quorum (3rd VM
...
[`docs/BACKLOG.md`](BACKLOG.md#priority-3--ha-svyatoslava--tier-1-hot-standby-blocked-on-2nd-vm--etcd-quorum--s3).
**Blocked on**:
- svyatoslava VM provisioning (operator)
