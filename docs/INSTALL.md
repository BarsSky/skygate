...
| `/admin/tailscale` looks enabled but `/admin/exit-nodes` says skygate is not on the tailnet | the daemon is not running while the SAVED settings look complete. Since v1.5.83 the Tailscale page says so explicitly and names the reason: (1) the container was created with `SKYGATE_TS_AUTHKEY_FILE=/dev/null` (or the variable unset), so the entrypoint skipped `tailscaled` at boot — that value is frozen at container creation, and the path saved on the page only affects the page and its **Start** button; press Start (it works in the running container) or recreate the container without the sentinel. (2) The last start failed with `CreateTUN("tailscale0") failed; /dev/net/tun does not exist` — add `devices: ["/dev/net/tun:/dev/net/tun"]` plus `NET_ADMIN`/`SYS_ADMIN` to the service and recreate it. Once real, `/admin/exit-nodes` stops routing relay management over the public address |
