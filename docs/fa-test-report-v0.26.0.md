...
### Test 7 — exit-node (relay-1) down

**Status**: PARTIAL — we cannot SSH to relay-1 from this VM.
...
because we couldn't make relay-1 go offline.
...
v0.27.0+ where the operator SSHes to relay-1 and runs `sudo
tailscale down` for 1 minute.

### Test 8 — subnet-router killed (FAIL — known gap)

```
$ # admin-subnet-router container is up, status=router_active
$ docker stop admin-subnet-router
...
$ docker start admin-subnet-router
...
2. **Exit-node alert test (medium)**: do a real SSH-to-relay-1-and-`tailscale
   down` test to verify the 5-min tick actually fires the Telegram
   alert. The exit-node monitor is in `internal/feature/exit_rules/`.
