...
publishes a special DNS record
  ...
  exitnode.skygate-subnet-<username>.tsnet.example.com
...
100.64.100.2) and the user's ACL permits access to it
...
### 4. Admin UI
  ...
  - Live FQDN preview: when a choice is set, the
    card shows `exitnode.skygate-subnet-<user>.tsnet.example.com`
...
100.64.100.2), which is in the relay's tailnet, which
is tagged `tag:exit-node`. So the user's tailnet
client can:

  1. DNS-resolve `exitnode.skygate-subnet-user1.tsnet.example.com`
  2. Get 100.64.100.2 (the exit-node's tailnet IP)
  3. Use it as the default route
  4. ACL permits the traffic (100.64.100.2 is
...
## Live verification
  ...
  - `/admin/users/1/subnet` (admin) — new
    "Preferred exit-node" card with the 3
    configured exit-nodes (relay-1, relay-2,
    relay-3) in the drop-down.
  - `headscale policy get` — when a user has a
    choice, the policy includes:
    ```
    "dns": {
      "extra_records": [
        {"name": "exitnode.skygate-subnet-user1.tsnet.example.com", "type": "A", "value": "100.64.100.2"}
      ]
    }
    ```
  - `tailscale dns` from any tailnet client —
    `exitnode.skygate-subnet-user1.tsnet.example.com`
