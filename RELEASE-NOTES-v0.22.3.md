...
For the 4 production users (admin, user1, guest, user2) this means
their subnets immediately flip to `active` on the next `/my/devices`
load. The user's Tailscale devices already work today via `100.64.100.0/10`
...
2. **`/my/devices` shows new card** for admin/user1 — CIDR +
   `active` pill
3. **`/admin/users` subnet column** for admin/user1 flips from
   `pending` (yellow) to `active` (green) on the first /my/devices
   load (admin sees it via /admin/users immediately)
4. **`/admin/users/{id}/subnet`** for admin: status pill + v0.22.3
...
- **v0.23.0** (Phase 1) — Infrastructure + admin pilot:
