...
- **lost access**: "the user will lose access to shared exit-nodes
  (relay-1/relay-2/relay-3 stay on the global plane) and to mesh
...
1. login as admin (smoke)
2. portal_users have per-user subnet CIDRs (admin: 10.0.1.0/24)
3. live headscale policy has per-user dst rules (10.0.1.0/24 + 10.0.35.0/24)
4. exit-nodes (relay-1, relay-2, relay-3) are in the global headscale
...
partial migration? Just admin, and also exit-nodes to
their own subnet, others stay in global?". The honest answer
(documented in this release's discussion):

- **admin → own headscale**: requires re-auth of all 5
  devices. After re-auth, admin can no longer use
  relay-1/relay-2/relay-3 (they stay in global). Tested:
  does NOT work, by design.
- **exit-nodes → own headscale**: requires re-key of all 3
  physical machines (relay-1, relay-2, relay-3). After
...
**wrong** tool for "give admin their own subnet + share
exit-nodes + mesh with user1". The right tool is the
...
but no migration of admin/user1/guest/user2 is planned
for v0.23.x.
