...
#     /home/admin/skygate/.env (we set it temporarily for
...
if [ -z "$TEST_NODE" ]; then
  ...
  /usr/bin/tailscale --socket=/tmp/ts-v0233-test/state/tailscale.sock \
    up --login-server=https://head.example.com --authkey="$PREAUTH" --accept-routes 2>&1 | head -3
  sleep 2
...
# Pick a tagged node (e.g. relay-1 / id=3) and check it still
...
echo "  relay-1 (tagged node id=3) expiry = ${TAGGED_EXPIRY}"
...
echo "  expirewatch renewals of relay-1: ${TAGGED_AUDIT} (should be 0)"
