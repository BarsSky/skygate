...
func renderHeadscaleOIDCBlock(v oidcExportValues, withSecret bool) string {
	redirect := firstOIDCRedirect(v.RedirectURIs)
	...
	b.WriteString("    - " + oidcHostOrPlaceholder(redirect) + "\n")
	b.WriteString("  auto_update: true\n")
	b.WriteString("  strip_email_domain: true\n")
	return b.String()
}

// firstOIDCRedirect returns the first entry of the comma-separated allowlist.
func firstOIDCRedirect(redirects string) string {
...
// oidcHostOrPlaceholder turns "https://head.example.com/oidc/callback" into
// "head.example.com" for allowed_domains, and keeps a placeholder when there is
// nothing to derive (an empty allowed_domains would make headscale reject every
// login).
func oidcHostOrPlaceholder(redirect string) string {
	if redirect == "" {
